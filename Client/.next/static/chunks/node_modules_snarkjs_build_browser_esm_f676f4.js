(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/node_modules_snarkjs_build_browser_esm_f676f4.js", {

"[project]/node_modules/snarkjs/build/browser.esm.js [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_import__("[project]/node_modules/ffjavascript/build/browser.esm.js [app-client] (ecmascript)");
...
class FastFile {
    ...
    _loadPage(p) {
        ...
        const P = new Promise((resolve, reject)=>{
            self1.pendingLoads.push({
                page: p,
                resolve: resolve,
                reject: reject
            });
        });
        self1.__statusPage("After Load request: ", p);
        return P;
    }
    __statusPage(s, p) {
        const logEntry = [];
        const self1 = this;
        if (!self1.logHistory) return;
        logEntry.push("==" + s + " " + p);
        let S = "";
        for(let i = 0; i < self1.pendingLoads.length; i++){
            if (self1.pendingLoads[i].page == p) S = S + " " + i;
        }
        if (S) logEntry.push("Pending loads:" + S);
        if (typeof self1.pages[p] != "undefined") {
            const page = self1.pages[p];
        ...
        logEntry.push("==");
        if (!self1.history[p]) self1.history[p] = [];
        self1.history[p].push(logEntry);
    }
    __printHistory(p) {
        const self1 = this;
        if (!self1.history[p]) console.log("Empty History ", p);
        console.log("History " + p);
        for(let i = 0; i < self1.history[p].length; i++){
            for(let j = 0; j < self1.history[p][i].length; j++){
                console.log("-> " + self1.history[p][i][j]);
            }
        }
    }
    _triggerLoad() {
        const self1 = this;
        if (self1.reading) return;
        if (self1.pendingLoads.length == 0) return;
        const pageIdxs = Object.keys(self1.pages);
        const deletablePages = [];
        for(let i = 0; i < pageIdxs.length; i++){
            const page = self1.pages[parseInt(pageIdxs[i])];
            if (page.dirty == false && page.pendingOps == 0 && !page.writing && !page.loading) deletablePages.push(parseInt(pageIdxs[i]));
        }
        let freePages = self1.maxPagesLoaded - pageIdxs.length;
        const ops = [];
        // while pending loads and
        //     the page is loaded or I can recover one.
        while(self1.pendingLoads.length > 0 && (typeof self1.pages[self1.pendingLoads[0].page] != "undefined" || freePages > 0 || deletablePages.length > 0)){
            const load = self1.pendingLoads.shift();
            if (typeof self1.pages[load.page] != "undefined") {
                self1.pages[load.page].pendingOps++;
                const idx = deletablePages.indexOf(load.page);
                if (idx >= 0) deletablePages.splice(idx, 1);
                if (self1.pages[load.page].loading) {
                    self1.pages[load.page].loading.push(load);
                } else {
                    load.resolve();
                }
                self1.__statusPage("After Load (cached): ", load.page);
            } else {
                if (freePages) {
                    freePages--;
                } else {
                    const fp = deletablePages.shift();
                    self1.__statusPage("Before Unload: ", fp);
                    self1.avBuffs.unshift(self1.pages[fp]);
                    delete self1.pages[fp];
                    self1.__statusPage("After Unload: ", fp);
                }
                if (load.page >= self1.totalPages) {
                    self1.pages[load.page] = getNewPage();
                    load.resolve();
                    self1.__statusPage("After Load (new): ", load.page);
                } else {
                    self1.reading = true;
                    self1.pages[load.page] = getNewPage();
                    self1.pages[load.page].loading = [
                        load
                    ];
                    ops.push(self1.fd.read(self1.pages[load.page].buff, 0, self1.pageSize, load.page * self1.pageSize).then((res)=>{
                        self1.pages[load.page].size = res.bytesRead;
                        const loading = self1.pages[load.page].loading;
                        delete self1.pages[load.page].loading;
                        for(let i = 0; i < loading.length; i++){
                            loading[i].resolve();
                        }
                        self1.__statusPage("After Load (loaded): ", load.page);
                        return res;
                    }, (err)=>{
                        load.reject(err);
                    }));
                    self1.__statusPage("After Load (loading): ", load.page);
                }
            }
        }
        ...
        Promise.all(ops).then(()=>{
            self1.reading = false;
            if (self1.pendingLoads.length > 0) setImmediate(self1._triggerLoad.bind(self1));
            self1._tryClose();
        });
        function getNewPage() {
            if (self1.avBuffs.length > 0) {
                const p = self1.avBuffs.shift();
                p.dirty = false;
                p.pendingOps = 1;
                p.size = 0;
                return p;
                ...
                return {
                    dirty: false,
                    buff: new Uint8Array(self1.pageSize),
                    pendingOps: 1,
                    size: 0
                };
            }
    ...
    _triggerWrite() {
        ...
        for(let i = 0; i < pageIdxs.length; i++){
            ...
            if (page.dirty) {
                ...
                page.writing = true;
                self1.writing = true;
                ops.push(self1.fd.write(page.buff, 0, page.size, parseInt(pageIdxs[i]) * self1.pageSize).then(()=>{
                    page.writing = false;
                    return;
                }, (err)=>{
        ...
        if (self1.writing) {
            Promise.all(ops).then(()=>{
                self1.writing = false;
                setImmediate(self1._triggerWrite.bind(self1));
                self1._tryClose();
                if (self1.pendingLoads.length > 0) setImmediate(self1._triggerLoad.bind(self1));
            });
        }
...
*/ if (typeof pos == "undefined") pos = self1.pos;
        self1.pos = pos + buff.byteLength;
        if (self1.totalSize < pos + buff.byteLength) self1.totalSize = pos + buff.byteLength;
        if (self1.pendingClose) throw new Error("Writing a closing file");
        const firstPage = Math.floor(pos / self1.pageSize);
        const lastPage = Math.floor((pos + buff.byteLength - 1) / self1.pageSize);
        const pagePromises = [];
        for(let i = firstPage; i <= lastPage; i++)pagePromises.push(self1._loadPage(i));
        self1._triggerLoad();
        let p = firstPage;
        let o = pos % self1.pageSize;
        let r = buff.byteLength;
        while(r > 0){
            await pagePromises[p - firstPage];
            const l = o + r > self1.pageSize ? self1.pageSize - o : r;
            const srcView = buff.slice(buff.byteLength - r, buff.byteLength - r + l);
            const dstView = new Uint8Array(self1.pages[p].buff.buffer, o, l);
            dstView.set(srcView);
            self1.pages[p].dirty = true;
            self1.pages[p].pendingOps--;
            self1.pages[p].size = Math.max(o + l, self1.pages[p].size);
            if (p >= self1.totalPages) {
                self1.totalPages = p + 1;
            }
            r = r - l;
            p++;
            o = 0;
            if (!self1.writing) setImmediate(self1._triggerWrite.bind(self1));
        }
    }
    async read(len, pos) {
        const self1 = this;
        let buff = new Uint8Array(len);
        await self1.readToBuffer(buff, 0, len, pos);
        return buff;
    }
    async readToBuffer(buffDst, offset, len, pos) {
        ...
        const self1 = this;
        if (len > self1.pageSize * self1.maxPagesLoaded * 0.8) {
            const cacheSize = Math.floor(len * 1.1);
            this.maxPagesLoaded = Math.floor(cacheSize / self1.pageSize) + 1;
        }
        if (typeof pos == "undefined") pos = self1.pos;
        self1.pos = pos + len;
        if (self1.pendingClose) throw new Error("Reading a closing file");
        const firstPage = Math.floor(pos / self1.pageSize);
        const lastPage = Math.floor((pos + len - 1) / self1.pageSize);
        const pagePromises = [];
        for(let i = firstPage; i <= lastPage; i++)pagePromises.push(self1._loadPage(i));
        self1._triggerLoad();
        let p = firstPage;
        ...
        let r = pos + len > self1.totalSize ? len - (pos + len - self1.totalSize) : len;
        while(r > 0){
            await pagePromises[p - firstPage];
            self1.__statusPage("After Await (read): ", p);
            // bytes to copy from this page
            const l = o + r > self1.pageSize ? self1.pageSize - o : r;
            const srcView = new Uint8Array(self1.pages[p].buff.buffer, self1.pages[p].buff.byteOffset + o, l);
            buffDst.set(srcView, offset + len - r);
            self1.pages[p].pendingOps--;
            self1.__statusPage("After Op done: ", p);
            r = r - l;
            p++;
            o = 0;
            if (self1.pendingLoads.length > 0) setImmediate(self1._triggerLoad.bind(self1));
    ...
    _tryClose() {
        const self1 = this;
        if (!self1.pendingClose) return;
        if (self1.error) {
            self1.pendingCloseReject(self1.error);
        }
        const p = self1._getDirtyPage();
        if (p >= 0 || self1.writing || self1.reading || self1.pendingLoads.length > 0) return;
        self1.pendingClose();
    }
    close() {
        const self1 = this;
        if (self1.pendingClose) throw new Error("Closing the file twice");
        return new Promise((resolve, reject)=>{
            self1.pendingClose = resolve;
            self1.pendingCloseReject = reject;
            self1._tryClose();
        }).then(()=>{
            self1.fd.close();
        }, (err)=>{
    ...
    async writeULE32(v, pos) {
        ...
        await self1.write(tmpBuff32, pos);
    }
    ...
    async writeULE64(v, pos) {
        ...
        await self1.write(tmpBuff64, pos);
    }
    async readULE32(pos) {
        const self1 = this;
        const b = await self1.read(4, pos);
    ...
    async readULE64(pos) {
        const self1 = this;
        const b = await self1.read(8, pos);
        const view = new Uint32Array(b.buffer);
        return view[1] * 0x100000000 + view[0];
    }
    async readString(pos) {
        const self1 = this;
        if (self1.pendingClose) {
            throw new Error("Reading a closing file");
        }
        let currentPosition = typeof pos == "undefined" ? self1.pos : pos;
        let currentPage = Math.floor(currentPosition / self1.pageSize);
        let endOfStringFound = false;
        let str = "";
        while(!endOfStringFound){
            //Read page
            let pagePromise = self1._loadPage(currentPage);
            self1._triggerLoad();
            await pagePromise;
            self1.__statusPage("After Await (read): ", currentPage);
            let offsetOnPage = currentPosition % self1.pageSize;
            const dataArray = new Uint8Array(self1.pages[currentPage].buff.buffer, self1.pages[currentPage].buff.byteOffset + offsetOnPage, self1.pageSize - offsetOnPage);
            ...
            if (endOfStringFound) {
                ...
                self1.pos = currentPage * this.pageSize + offsetOnPage + indexEndOfString + 1;
            } else {
                str += new TextDecoder().decode(dataArray);
                self1.pos = currentPage * this.pageSize + offsetOnPage + dataArray.length;
            }
            self1.pages[currentPage].pendingOps--;
            self1.__statusPage("After Op done: ", currentPage);
            currentPosition = self1.pos;
            currentPage++;
            if (self1.pendingLoads.length > 0) setImmediate(self1._triggerLoad.bind(self1));
        }
        return str;
...
class MemFile {
    ...
    async writeULE32(v, pos) {
        ...
        await self1.write(tmpBuff32$1, pos);
    }
    async writeUBE32(v, pos) {
        const self1 = this;
    ...
    async writeULE64(v, pos) {
        ...
        await self1.write(tmpBuff64$1, pos);
    }
    async readULE32(pos) {
        const self1 = this;
        const b = await self1.read(4, pos);
    ...
    async readString(pos) {
        ...
        const dataArray = new Uint8Array(self1.o.data.buffer, currentPosition, this.totalSize - currentPosition);
        let indexEndOfString = dataArray.findIndex((element)=>element === 0);
        let endOfStringFound = indexEndOfString !== -1;
        let str = "";
        if (endOfStringFound) {
            str = new TextDecoder().decode(dataArray.slice(0, indexEndOfString));
            self1.pos = currentPosition + indexEndOfString + 1;
        } else {
            self1.pos = currentPosition;
        }
        return str;
    }
}
...
class BigMemFile {
    ...
    async write(buff, pos) {
        ...
        while(r > 0){
            ...
            const srcView = buff.slice(buff.byteLength - r, buff.byteLength - r + l);
            const dstView = new Uint8Array(self1.o.data[p].buffer, o, l);
            dstView.set(srcView);
            r = r - l;
            p++;
            o = 0;
    ...
    async readToBuffer(buffDst, offset, len, pos) {
        ...
        while(r > 0){
            // bytes to copy from this page
            const l = o + r > PAGE_SIZE ? PAGE_SIZE - o : r;
            const srcView = new Uint8Array(self1.o.data[p].buffer, o, l);
            buffDst.set(srcView, offset + len - r);
            r = r - l;
            p++;
            o = 0;
    ...
    async readString(pos) {
        ...
        while(!endOfStringFound){
            ...
            if (self1.o.data[currentPage] === undefined) {
                throw new Error("ERROR");
            }
            let readLength = Math.min(fixedSize, self1.o.data[currentPage].length - offsetOnPage);
            const dataArray = new Uint8Array(self1.o.data[currentPage].buffer, offsetOnPage, readLength);
            let indexEndOfString = dataArray.findIndex((element)=>element === 0);
            endOfStringFound = indexEndOfString !== -1;
            if (endOfStringFound) {
                str += new TextDecoder().decode(dataArray.slice(0, indexEndOfString));
                self1.pos = currentPage * PAGE_SIZE + offsetOnPage + indexEndOfString + 1;
            } else {
                str += new TextDecoder().decode(dataArray);
                self1.pos = currentPage * PAGE_SIZE + offsetOnPage + dataArray.length;
            }
            currentPosition = self1.pos;
        }
        return str;
...
const O_RDWR = 2;
const O_RDONLY = 0;
/* global fetch */ const DEFAULT_CACHE_SIZE = 1 << 16;
...
async function writeBigInt(fd, n, n8, pos) {
    ...
    __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toRprLE(buff, 0, n, n8);
    await fd.write(buff, pos);
}
async function readBigInt(fd, n8, pos) {
    const buff = await fd.read(n8, pos);
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromRprLE(buff, 0, n8);
}
...
async function readSection(fd, sections, idSection, offset, length) {
    ...
    if (length < 1 << 30) {
        ...
        buff = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](length);
    }
...
const bls12381r$1 = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e("73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001", 16);
const bn128r$1 = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e("21888242871839275222246405745257275088548364400416034343698204186575808495617");
const bls12381q = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e("1a0111ea397fe69a4b1ba7b6434bacd764774b84f38512bf6730d2a0f6b0f6241eabfffeb153ffffb9feffffffffaaab", 16);
const bn128q = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e("21888242871839275222246405745257275088696311157297823662689037894645226208583");
async function getCurveFromR(r, options) {
    ...
    if (__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(r, bn128r$1)) {
        curve1 = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["buildBn128"])(singleThread);
    } else if (__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(r, bls12381r$1)) {
        curve1 = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["buildBls12381"])(singleThread);
    } else {
        throw new Error(`Curve not supported: ${__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toString(r)}`);
    }
    return curve1;
}
async function getCurveFromQ(q, options) {
    let curve1;
    let singleThread = options && options.singleThread;
    if (__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(q, bn128q)) {
        curve1 = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["buildBn128"])(singleThread);
    } else if (__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(q, bls12381q)) {
        curve1 = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["buildBls12381"])(singleThread);
    } else {
        throw new Error(`Curve not supported: ${__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toString(q)}`);
    }
    return curve1;
}
...
async function sameRatio$2(curve1, g1s, g1sx, g2s, g2sx) {
    if (curve1.G1.isZero(g1s)) return false;
    if (curve1.G1.isZero(g1sx)) return false;
    if (curve1.G2.isZero(g2s)) return false;
    if (curve1.G2.isZero(g2sx)) return false;
    // return curve.F12.eq(curve.pairing(g1s, g2sx), curve.pairing(g1sx, g2s));
    const res = await curve1.pairingEq(g1s, g2sx, curve1.G1.neg(g1sx), g2s);
    return res;
}
function askEntropy() {
    {
...
async function getRandomRng(entropy) {
    ...
    const rng = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ChaCha"](seed);
    return rng;
}
...
*/ async function writeHeader(fd, zkey) {
    ...
    ///////////
    const curve1 = await getCurveFromQ(zkey.q);
    await startWriteSection(fd, 2);
    const primeQ = curve1.q;
    const n8q = (Math.floor((__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].bitLength(primeQ) - 1) / 64) + 1) * 8;
    const primeR = curve1.r;
    const n8r = (Math.floor((__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].bitLength(primeR) - 1) / 64) + 1) * 8;
    ...
    await writeG1(fd, curve1, zkey.vk_alpha_1);
    await writeG1(fd, curve1, zkey.vk_beta_1);
    await writeG2(fd, curve1, zkey.vk_beta_2);
    await writeG2(fd, curve1, zkey.vk_gamma_2);
    await writeG1(fd, curve1, zkey.vk_delta_1);
    await writeG2(fd, curve1, zkey.vk_delta_2);
    await endWriteSection(fd);
}
async function writeG1(fd, curve1, p) {
    const buff = new Uint8Array(curve1.G1.F.n8 * 2);
    curve1.G1.toRprLEM(buff, 0, p);
    await fd.write(buff);
}
async function writeG2(fd, curve1, p) {
    const buff = new Uint8Array(curve1.G2.F.n8 * 2);
    curve1.G2.toRprLEM(buff, 0, p);
    await fd.write(buff);
}
async function readG1(fd, curve1, toObject) {
    const buff = await fd.read(curve1.G1.F.n8 * 2);
    const res = curve1.G1.fromRprLEM(buff, 0);
    return toObject ? curve1.G1.toObject(res) : res;
}
async function readG2(fd, curve1, toObject) {
    const buff = await fd.read(curve1.G2.F.n8 * 2);
    const res = curve1.G2.fromRprLEM(buff, 0);
    return toObject ? curve1.G2.toObject(res) : res;
}
...
async function readZKey(fileName, toObject) {
    ...
    const Fr = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["F1Field"](zkey.r);
    const Rr = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].mod(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].shl(1, zkey.n8r * 8), zkey.r);
    const Rri = Fr.inv(Rr);
    const Rri2 = Fr.mul(Rri, Rri);
    let curve1 = await getCurveFromQ(zkey.q);
    // Read IC Section
    ///////////
    await startReadUniqueSection(fd, sections, 3);
    zkey.IC = [];
    for(let i = 0; i <= zkey.nPublic; i++){
        const P = await readG1(fd, curve1, toObject);
        zkey.IC.push(P);
    }
    await endReadSection(fd);
    // Read Coefs
    ...
    for(let i = 0; i < zkey.nVars; i++){
        const A = await readG1(fd, curve1, toObject);
        zkey.A[i] = A;
    }
    await endReadSection(fd);
    // Read B1
    ...
    zkey.B1 = [];
    for(let i = 0; i < zkey.nVars; i++){
        const B1 = await readG1(fd, curve1, toObject);
        zkey.B1[i] = B1;
    }
    await endReadSection(fd);
    // Read B2 points
    ...
    zkey.B2 = [];
    for(let i = 0; i < zkey.nVars; i++){
        const B2 = await readG2(fd, curve1, toObject);
        zkey.B2[i] = B2;
    }
    await endReadSection(fd);
    // Read C points
    ...
    for(let i = zkey.nPublic + 1; i < zkey.nVars; i++){
        const C = await readG1(fd, curve1, toObject);
        zkey.C[i] = C;
    }
    await endReadSection(fd);
    // Read H points
    ...
    for(let i = 0; i < zkey.domainSize; i++){
        const H = await readG1(fd, curve1, toObject);
        zkey.hExps.push(H);
    }
    await endReadSection(fd);
    await fd.close();
...
async function readContribution$1(fd, curve1, toObject) {
    const c = {
        delta: {}
    };
    c.deltaAfter = await readG1(fd, curve1, toObject);
    c.delta.g1_s = await readG1(fd, curve1, toObject);
    c.delta.g1_sx = await readG1(fd, curve1, toObject);
    c.delta.g2_spx = await readG2(fd, curve1, toObject);
...
async function readMPCParams(fd, curve1, sections) {
    ...
    for(let i = 0; i < n; i++){
        const c = await readContribution$1(fd, curve1);
        res.contributions.push(c);
    }
    await endReadSection(fd);
    return res;
}
async function writeContribution$1(fd, curve1, c) {
    await writeG1(fd, curve1, c.deltaAfter);
    await writeG1(fd, curve1, c.delta.g1_s);
    await writeG1(fd, curve1, c.delta.g1_sx);
    await writeG2(fd, curve1, c.delta.g2_spx);
...
async function writeMPCParams(fd, curve1, mpcParams) {
    await startWriteSection(fd, 10);
    await fd.write(mpcParams.csHash);
    await fd.writeULE32(mpcParams.contributions.length);
    for(let i = 0; i < mpcParams.contributions.length; i++){
        await writeContribution$1(fd, curve1, mpcParams.contributions[i]);
    }
    await endWriteSection(fd);
}
function hashG1(hasher, curve1, p) {
    const buff = new Uint8Array(curve1.G1.F.n8 * 2);
    curve1.G1.toRprUncompressed(buff, 0, p);
    hasher.update(buff);
}
function hashG2(hasher, curve1, p) {
    const buff = new Uint8Array(curve1.G2.F.n8 * 2);
    curve1.G2.toRprUncompressed(buff, 0, p);
    hasher.update(buff);
}
function hashPubKey(hasher, curve1, c) {
    hashG1(hasher, curve1, c.deltaAfter);
    hashG1(hasher, curve1, c.delta.g1_s);
    hashG1(hasher, curve1, c.delta.g1_sx);
    hashG2(hasher, curve1, c.delta.g2_spx);
...
*/ async function write(fd, witness, prime) {
    await startWriteSection(fd, 1);
    const n8 = (Math.floor((__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].bitLength(prime) - 1) / 64) + 1) * 8;
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { stringifyBigInts: stringifyBigInts$4 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
async function groth16Prove(zkeyFileName, witnessFileName, logger, options) {
    ...
    if (!__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(zkey.r, wtns.q)) {
        throw new Error("Curve of the witness does not match the curve of the proving key");
    }
    if (wtns.nWitness != zkey.nVars) {
        throw new Error(`Invalid witness length. Circuit: ${zkey.nVars}, witness: ${wtns.nWitness}`);
    }
    const curve1 = zkey.curve;
    const Fr = curve1.Fr;
    const G1 = curve1.G1;
    const G2 = curve1.G2;
    ...
    const [buffA_T, buffB_T, buffC_T] = await buildABC1(curve1, zkey, buffWitness, buffCoeffs, logger);
    const inc = power == Fr.s ? curve1.Fr.shift : curve1.Fr.w[power + 1];
    ...
    const buffPodd_T = await joinABC(curve1, zkey, buffAodd_T, buffBodd_T, buffCodd_T, logger);
    let proof = {};
    if (logger) logger.debug("Reading A Points");
    const buffBasesA = await readSection(fdZKey, sectionsZKey, 5);
    proof.pi_a = await curve1.G1.multiExpAffine(buffBasesA, buffWitness, logger, "multiexp A");
    if (logger) logger.debug("Reading B1 Points");
    const buffBasesB1 = await readSection(fdZKey, sectionsZKey, 6);
    let pib1 = await curve1.G1.multiExpAffine(buffBasesB1, buffWitness, logger, "multiexp B1");
    if (logger) logger.debug("Reading B2 Points");
    const buffBasesB2 = await readSection(fdZKey, sectionsZKey, 7);
    proof.pi_b = await curve1.G2.multiExpAffine(buffBasesB2, buffWitness, logger, "multiexp B2");
    if (logger) logger.debug("Reading C Points");
    const buffBasesC = await readSection(fdZKey, sectionsZKey, 8);
    proof.pi_c = await curve1.G1.multiExpAffine(buffBasesC, buffWitness.slice((zkey.nPublic + 1) * curve1.Fr.n8), logger, "multiexp C");
    if (logger) logger.debug("Reading H Points");
    const buffBasesH = await readSection(fdZKey, sectionsZKey, 9);
    const resH = await curve1.G1.multiExpAffine(buffBasesH, buffPodd_T, logger, "multiexp H");
    const r = curve1.Fr.random();
    const s = curve1.Fr.random();
    ...
    for(let i = 1; i <= zkey.nPublic; i++){
        ...
        publicSignals.push(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromRprLE(b));
    }
    proof.pi_a = G1.toObject(G1.toAffine(proof.pi_a));
    proof.pi_b = G2.toObject(G2.toAffine(proof.pi_b));
    proof.pi_c = G1.toObject(G1.toAffine(proof.pi_c));
    proof.protocol = "groth16";
    proof.curve = curve1.name;
    await fdZKey.close();
    await fdWtns.close();
...
async function buildABC1(curve1, zkey, witness, coeffs, logger) {
    const n8 = curve1.Fr.n8;
    const sCoef = 4 * 3 + zkey.n8r;
    const nCoef = (coeffs.byteLength - 4) / sCoef;
    const outBuffA = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](zkey.domainSize * n8);
    const outBuffB = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](zkey.domainSize * n8);
    const outBuffC = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](zkey.domainSize * n8);
    const outBuf = [
        outBuffA,
        outBuffB
    ];
    for(let i = 0; i < nCoef; i++){
        ...
        outBuf[m].set(curve1.Fr.add(outBuf[m].slice(c * n8, c * n8 + n8), curve1.Fr.mul(coef, witness.slice(s * n8, s * n8 + n8))), c * n8);
    }
    for(let i = 0; i < zkey.domainSize; i++){
        if (logger && i % 1000000 == 0) logger.debug(`QAP C: ${i}/${zkey.domainSize}`);
        outBuffC.set(curve1.Fr.mul(outBuffA.slice(i * n8, i * n8 + n8), outBuffB.slice(i * n8, i * n8 + n8)), i * n8);
    }
    return [
        outBuffA,
        outBuffB,
...
*/ async function joinABC(curve1, zkey, a, b, c, logger) {
    const MAX_CHUNK_SIZE = 1 << 22;
    const n8 = curve1.Fr.n8;
    const nElements = Math.floor(a.byteLength / curve1.Fr.n8);
    const promises = [];
    for(let i = 0; i < nElements; i += MAX_CHUNK_SIZE){
        ...
        promises.push(curve1.tm.queueAction(task));
    }
    const result = await Promise.all(promises);
    let outBuff;
    if (a instanceof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]) {
        outBuff = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](a.byteLength);
    } else {
...
/* globals WebAssembly */ async function builder(code, options) {
    let instance;
    let wc;
    let memory;
    options = options || {};
            ...
            runtime: {
                ...
                showSharedRWMemory: function() {
                    ...
                    if (majorVersion >= 2 && (minorVersion >= 1 || patchVersion >= 7)) {
                        ...
                        const msg = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromArray(arr, 0x100000000).toString();
                        msgStr += msg;
                    } else {
                        console.log(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromArray(arr, 0x100000000));
                    }
                },
...
class WitnessCalculatorCircom1 {
    constructor(memory, instance, sanityCheck){
        ...
        this.prime = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromArray(arr, 0x100000000);
        this.Fr = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["F1Field"](this.prime);
        this.mask32 = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromString("FFFFFFFF", 16);
        this.NVars = this.instance.exports.getNVars();
        this.n64 = Math.floor((this.Fr.bitLength - 1) / 64) + 1;
        this.R = this.Fr.e(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].shiftLeft(1, this.n64 * 64));
    ...
    async calculateWitness(input, sanityCheck) {
        const self1 = this;
        const old0 = self1.i32[0];
        const w = [];
        await self1._doCalculateWitness(input, sanityCheck);
        for(let i = 0; i < self1.NVars; i++){
            const pWitness = self1.instance.exports.getPWitness(i);
            w.push(self1.getFr(pWitness));
        }
        self1.i32[0] = old0;
        return w;
    }
    async calculateBinWitness(input, sanityCheck) {
        ...
        const old0 = self1.i32[0];
        await self1._doCalculateWitness(input, sanityCheck);
        const pWitnessBuffer = self1.instance.exports.getWitnessBuffer();
        self1.i32[0] = old0;
        const buff = self1.memory.buffer.slice(pWitnessBuffer, pWitnessBuffer + self1.NVars * self1.n64 * 8);
    ...
    getFr(p) {
        const self1 = this;
        const idx = p >> 2;
        if (self1.i32[idx + 1] & 0x80000000) {
            const arr = new Array(self1.n32);
            for(let i = 0; i < self1.n32; i++){
                arr[self1.n32 - 1 - i] = self1.i32[idx + 2 + i];
            }
            const res = self1.Fr.e(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromArray(arr, 0x100000000));
            if (self1.i32[idx + 1] & 0x40000000) {
                return fromMontgomery(res);
            } else {
                return res;
            }
        } else {
            if (self1.i32[idx] & 0x80000000) {
                return self1.Fr.e(self1.i32[idx] - 0x100000000);
            } else {
                return self1.Fr.e(self1.i32[idx]);
            }
        }
        function fromMontgomery(n) {
            return self1.Fr.mul(self1.RInv, n);
        }
    }
    setFr(p, v) {
        const self1 = this;
        v = self1.Fr.e(v);
        const minShort = self1.Fr.neg(self1.Fr.e("80000000", 16));
        const maxShort = self1.Fr.e("7FFFFFFF", 16);
        if (self1.Fr.geq(v, minShort) && self1.Fr.leq(v, maxShort)) {
            let a;
            if (self1.Fr.geq(v, self1.Fr.zero)) {
                a = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toNumber(v);
            } else {
                a = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toNumber(self1.Fr.sub(v, minShort));
                a = a - 0x80000000;
                a = 0x100000000 + a;
            }
            self1.i32[p >> 2] = a;
        ...
        self1.i32[p >> 2] = 0;
        self1.i32[(p >> 2) + 1] = 0x80000000;
        const arr = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toArray(v, 0x100000000);
        for(let i = 0; i < self1.n32; i++){
            const idx = arr.length - 1 - i;
            if (idx >= 0) {
                self1.i32[(p >> 2) + 2 + i] = arr[idx];
            } else {
                self1.i32[(p >> 2) + 2 + i] = 0;
            }
        }
...
class WitnessCalculatorCircom2 {
    ...
    async calculateWitness(input, sanityCheck) {
        ...
        for(let i = 0; i < this.witnessSize; i++){
            ...
            w.push(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromArray(arr, 0x100000000));
        }
        return w;
    }
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$b } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$a } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
/*
    ...
    You should have received a copy of the GNU General Public License along with
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$9 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
*/ const vk_verifier = unstringifyBigInts$9(_vk_verifier);
    ...
    const curve1 = await getCurveFromName(vk_verifier.curve);
    const IC0 = curve1.G1.fromObject(vk_verifier.IC[0]);
    const IC = new Uint8Array(curve1.G1.F.n8 * 2 * publicSignals.length);
    const w = new Uint8Array(curve1.Fr.n8 * publicSignals.length);
    if (!publicInputsAreValid$1(curve1, publicSignals)) {
        if (logger) logger.error("Public inputs are not valid.");
        return false;
    }
    for(let i = 0; i < publicSignals.length; i++){
        const buffP = curve1.G1.fromObject(vk_verifier.IC[i + 1]);
        IC.set(buffP, i * curve1.G1.F.n8 * 2);
        __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toRprLE(w, curve1.Fr.n8 * i, publicSignals[i], curve1.Fr.n8);
    }
    let cpub = await curve1.G1.multiExpAffine(IC, w);
    cpub = curve1.G1.add(cpub, IC0);
    const pi_a = curve1.G1.fromObject(proof.pi_a);
    const pi_b = curve1.G2.fromObject(proof.pi_b);
    const pi_c = curve1.G1.fromObject(proof.pi_c);
    if (!isWellConstructed$1(curve1, {
        pi_a,
        pi_b,
        pi_c
    })) {
    ...
    const vk_gamma_2 = curve1.G2.fromObject(vk_verifier.vk_gamma_2);
    const vk_delta_2 = curve1.G2.fromObject(vk_verifier.vk_delta_2);
    const vk_alpha_1 = curve1.G1.fromObject(vk_verifier.vk_alpha_1);
    const vk_beta_2 = curve1.G2.fromObject(vk_verifier.vk_beta_2);
    const res = await curve1.pairingEq(curve1.G1.neg(pi_a), pi_b, cpub, vk_gamma_2, pi_c, vk_delta_2, vk_alpha_1, vk_beta_2);
    if (!res) {
...
function isWellConstructed$1(curve1, proof) {
    const G1 = curve1.G1;
    const G2 = curve1.G2;
    return G1.isValid(proof.pi_a) && G2.isValid(proof.pi_b) && G1.isValid(proof.pi_c);
}
function publicInputsAreValid$1(curve1, publicInputs) {
    for(let i = 0; i < publicInputs.length; i++){
        if (!__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].lt(publicInputs[i], curve1.r)) {
            return false;
        }
    }
    return true;
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$8 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
function p256$2(n) {
...
*/ function hashToG2(curve1, hash) {
    ...
    const rng = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ChaCha"](seed);
    const g2_sp = curve1.G2.fromRng(rng);
    return g2_sp;
}
function getG2sp(curve1, persinalization, challenge, g1s, g1sx) {
    ...
    h.update(b1);
    h.update(challenge);
    const b3 = curve1.G1.toUncompressed(g1s);
    h.update(b3);
    const b4 = curve1.G1.toUncompressed(g1sx);
    h.update(b4);
    const hash = h.digest();
    return hashToG2(curve1, hash);
}
function calculatePubKey(k, curve1, personalization, challengeHash, rng) {
    k.g1_s = curve1.G1.toAffine(curve1.G1.fromRng(rng));
    k.g1_sx = curve1.G1.toAffine(curve1.G1.timesFr(k.g1_s, k.prvKey));
    k.g2_sp = curve1.G2.toAffine(getG2sp(curve1, personalization, challengeHash, k.g1_s, k.g1_sx));
    k.g2_spx = curve1.G2.toAffine(curve1.G2.timesFr(k.g2_sp, k.prvKey));
    return k;
}
function createPTauKey(curve1, challengeHash, rng) {
    const key = {
        tau: {},
        alpha: {},
        beta: {}
    };
    key.tau.prvKey = curve1.Fr.fromRng(rng);
    key.alpha.prvKey = curve1.Fr.fromRng(rng);
    key.beta.prvKey = curve1.Fr.fromRng(rng);
    calculatePubKey(key.tau, curve1, 0, challengeHash, rng);
    calculatePubKey(key.alpha, curve1, 1, challengeHash, rng);
    calculatePubKey(key.beta, curve1, 2, challengeHash, rng);
    return key;
}
/*
    Copyright 2018 0KIMS association.
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ async function writePTauHeader(fd, curve1, power, ceremonyPower) {
    // Write the header
    ///////////
    ...
    await fd.writeULE64(0); // Temporally set to 0 length
    await fd.writeULE32(curve1.F1.n64 * 8);
    const buff = new Uint8Array(curve1.F1.n8);
    __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toRprLE(buff, 0, curve1.q, curve1.F1.n8);
    await fd.write(buff);
...
async function readPTauHeader(fd, sections) {
    ...
    const q = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromRprLE(buff);
    const curve1 = await getCurveFromQ(q);
    if (curve1.F1.n64 * 8 != n8) throw new Error(fd.fileName + ": Invalid size");
...
async function readPtauPubKey(fd, curve1, montgomery) {
    const buff = await fd.read(curve1.F1.n8 * 2 * 6 + curve1.F2.n8 * 2 * 3);
    return fromPtauPubKeyRpr(buff, 0, curve1, montgomery);
}
function fromPtauPubKeyRpr(buff, pos, curve1, montgomery) {
    const key = {
        tau: {},
        alpha: {},
        beta: {}
    ...
    function readG1() {
        let p;
        if (montgomery) {
            p = curve1.G1.fromRprLEM(buff, pos);
        } else {
            p = curve1.G1.fromRprUncompressed(buff, pos);
        }
        pos += curve1.G1.F.n8 * 2;
        return p;
    }
    function readG2() {
        let p;
        if (montgomery) {
            p = curve1.G2.fromRprLEM(buff, pos);
        } else {
            p = curve1.G2.fromRprUncompressed(buff, pos);
        }
        pos += curve1.G2.F.n8 * 2;
        return p;
    }
}
function toPtauPubKeyRpr(buff, pos, curve1, key, montgomery) {
    ...
    async function writeG1(p) {
        if (montgomery) {
            curve1.G1.toRprLEM(buff, pos, p);
        } else {
            curve1.G1.toRprUncompressed(buff, pos, p);
        }
        pos += curve1.F1.n8 * 2;
    }
    async function writeG2(p) {
        if (montgomery) {
            curve1.G2.toRprLEM(buff, pos, p);
        } else {
            curve1.G2.toRprUncompressed(buff, pos, p);
        }
        pos += curve1.F2.n8 * 2;
    }
    return buff;
}
async function writePtauPubKey(fd, curve1, key, montgomery) {
    const buff = new Uint8Array(curve1.F1.n8 * 2 * 6 + curve1.F2.n8 * 2 * 3);
    toPtauPubKeyRpr(buff, 0, curve1, key, montgomery);
    await fd.write(buff);
}
async function readContribution(fd, curve1) {
    const c = {};
    ...
    c.key = await readPtauPubKey(fd, curve1, true);
    c.partialHash = await fd.read(216);
    c.nextChallenge = await fd.read(64);
    c.type = await fd.readULE32();
    const buffV = new Uint8Array(curve1.G1.F.n8 * 2 * 6 + curve1.G2.F.n8 * 2 * 3);
    toPtauPubKeyRpr(buffV, 0, curve1, c.key, false);
    ...
    async function readG1() {
        const pBuff = await fd.read(curve1.G1.F.n8 * 2);
        return curve1.G1.fromRprLEM(pBuff);
    }
    async function readG2() {
        const pBuff = await fd.read(curve1.G2.F.n8 * 2);
        return curve1.G2.fromRprLEM(pBuff);
    }
...
async function readContributions(fd, curve1, sections) {
    ...
    for(let i = 0; i < nContributions; i++){
        const c = await readContribution(fd, curve1);
        c.id = i + 1;
...
async function writeContribution(fd, curve1, contribution) {
    const buffG1 = new Uint8Array(curve1.F1.n8 * 2);
    const buffG2 = new Uint8Array(curve1.F2.n8 * 2);
    await writeG1(contribution.tauG1);
    await writeG2(contribution.tauG2);
    await writeG1(contribution.alphaG1);
    await writeG1(contribution.betaG1);
    await writeG2(contribution.betaG2);
    await writePtauPubKey(fd, curve1, contribution.key, true);
    ...
    async function writeG1(p) {
        curve1.G1.toRprLEM(buffG1, 0, p);
        await fd.write(buffG1);
    }
    async function writeG2(p) {
        curve1.G2.toRprLEM(buffG2, 0, p);
        await fd.write(buffG2);
    }
}
async function writeContributions(fd, curve1, contributions) {
    await fd.writeULE32(7); // Header type
    const pContributionsSize = fd.pos;
    await fd.writeULE64(0); // Temporally set to 0 length
    await fd.writeULE32(contributions.length);
    for(let i = 0; i < contributions.length; i++){
        await writeContribution(fd, curve1, contributions[i]);
    }
...
function calculateFirstChallengeHash(curve1, power, logger) {
    if (logger) logger.debug("Calculating First Challenge Hash");
    const hasher = new blake2bWasm.exports(64);
    const vG1 = new Uint8Array(curve1.G1.F.n8 * 2);
    const vG2 = new Uint8Array(curve1.G2.F.n8 * 2);
    curve1.G1.toRprUncompressed(vG1, 0, curve1.G1.g);
    curve1.G2.toRprUncompressed(vG2, 0, curve1.G2.g);
...
async function keyFromBeacon(curve1, challengeHash, beaconHash, numIterationsExp) {
    const rng = await rngFromBeaconParams(beaconHash, numIterationsExp);
    const key = createPTauKey(curve1, challengeHash, rng);
    return key;
}
/*
    Copyright 2018 0KIMS association.
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ async function newAccumulator(curve1, power, fileName, logger) {
    await blake2bWasm.exports.ready();
    const fd = await createBinFile(fileName, "ptau", 1, 7);
    await writePTauHeader(fd, curve1, power, 0);
    const buffG1 = curve1.G1.oneAffine;
    const buffG2 = curve1.G2.oneAffine;
    // Write tauG1
    ///////////
    await startWriteSection(fd, 2);
    ...
    await endWriteSection(fd);
    await fd.close();
    const firstChallengeHash = calculateFirstChallengeHash(curve1, power, logger);
...
async function exportChallenge(pTauFilename, challengeFilename, logger) {
    ...
    const { curve: curve1, power } = await readPTauHeader(fdFrom, sections);
    const contributions = await readContributions(fdFrom, curve1, sections);
    let lastResponseHash, curChallengeHash;
    if (contributions.length == 0) {
        lastResponseHash = blake2bWasm.exports(64).digest();
        curChallengeHash = calculateFirstChallengeHash(curve1, power);
    } else {
...
*/ async function importResponse(oldPtauFilename, contributionFilename, newPTauFilename, name, importPoints, logger) {
    ...
    const { curve: curve1, power } = await readPTauHeader(fdOld, sections);
    const contributions = await readContributions(fdOld, curve1, sections);
    const currentContribution = {};
    if (name) currentContribution.name = name;
    const sG1 = curve1.F1.n8 * 2;
    const scG1 = curve1.F1.n8; // Compressed size
    const sG2 = curve1.F2.n8 * 2;
    const scG2 = curve1.F2.n8; // Compressed size
    ...
    if (contributions.length > 0) {
        ...
        lastChallengeHash = calculateFirstChallengeHash(curve1, power, logger);
    }
    const fdNew = await createBinFile(newPTauFilename, "ptau", 1, importPoints ? 7 : 2);
    await writePTauHeader(fdNew, curve1, power);
    ...
    const buffKey = await fdResponse.read(curve1.F1.n8 * 2 * 6 + curve1.F2.n8 * 2 * 3);
    currentContribution.key = fromPtauPubKeyRpr(buffKey, 0, curve1, false);
    ...
    await writeContributions(fdNew, curve1, contributions);
...
async function verifyContribution(curve1, cur, prev, logger) {
    let sr;
    if (cur.type == 1) {
        const beaconKey = await keyFromBeacon(curve1, prev.nextChallenge, cur.beaconHash, cur.numIterationsExp);
        if (!curve1.G1.eq(cur.key.tau.g1_s, beaconKey.tau.g1_s)) {
            if (logger) logger.error(`BEACON key (tauG1_s) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G1.eq(cur.key.tau.g1_sx, beaconKey.tau.g1_sx)) {
            if (logger) logger.error(`BEACON key (tauG1_sx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G2.eq(cur.key.tau.g2_spx, beaconKey.tau.g2_spx)) {
            if (logger) logger.error(`BEACON key (tauG2_spx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G1.eq(cur.key.alpha.g1_s, beaconKey.alpha.g1_s)) {
            if (logger) logger.error(`BEACON key (alphaG1_s) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G1.eq(cur.key.alpha.g1_sx, beaconKey.alpha.g1_sx)) {
            if (logger) logger.error(`BEACON key (alphaG1_sx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G2.eq(cur.key.alpha.g2_spx, beaconKey.alpha.g2_spx)) {
            if (logger) logger.error(`BEACON key (alphaG2_spx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G1.eq(cur.key.beta.g1_s, beaconKey.beta.g1_s)) {
            if (logger) logger.error(`BEACON key (betaG1_s) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G1.eq(cur.key.beta.g1_sx, beaconKey.beta.g1_sx)) {
            if (logger) logger.error(`BEACON key (betaG1_sx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
        if (!curve1.G2.eq(cur.key.beta.g2_spx, beaconKey.beta.g2_spx)) {
            if (logger) logger.error(`BEACON key (betaG2_spx) is not generated correctly in challenge #${cur.id}  ${cur.name || ""}`);
            return false;
        }
    }
    cur.key.tau.g2_sp = curve1.G2.toAffine(getG2sp(curve1, 0, prev.nextChallenge, cur.key.tau.g1_s, cur.key.tau.g1_sx));
    cur.key.alpha.g2_sp = curve1.G2.toAffine(getG2sp(curve1, 1, prev.nextChallenge, cur.key.alpha.g1_s, cur.key.alpha.g1_sx));
    cur.key.beta.g2_sp = curve1.G2.toAffine(getG2sp(curve1, 2, prev.nextChallenge, cur.key.beta.g1_s, cur.key.beta.g1_sx));
    sr = await sameRatio$1(curve1, cur.key.tau.g1_s, cur.key.tau.g1_sx, cur.key.tau.g2_sp, cur.key.tau.g2_spx);
    if (sr !== true) {
        if (logger) logger.error("INVALID key (tau) in challenge #" + cur.id);
        return false;
    }
    sr = await sameRatio$1(curve1, cur.key.alpha.g1_s, cur.key.alpha.g1_sx, cur.key.alpha.g2_sp, cur.key.alpha.g2_spx);
    if (sr !== true) {
        if (logger) logger.error("INVALID key (alpha) in challenge #" + cur.id);
        return false;
    }
    sr = await sameRatio$1(curve1, cur.key.beta.g1_s, cur.key.beta.g1_sx, cur.key.beta.g2_sp, cur.key.beta.g2_spx);
    if (sr !== true) {
        if (logger) logger.error("INVALID key (beta) in challenge #" + cur.id);
        return false;
    }
    sr = await sameRatio$1(curve1, prev.tauG1, cur.tauG1, cur.key.tau.g2_sp, cur.key.tau.g2_spx);
    if (sr !== true) {
        if (logger) logger.error("INVALID tau*G1. challenge #" + cur.id + " It does not follow the previous contribution");
        return false;
    }
    sr = await sameRatio$1(curve1, cur.key.tau.g1_s, cur.key.tau.g1_sx, prev.tauG2, cur.tauG2);
    if (sr !== true) {
        if (logger) logger.error("INVALID tau*G2. challenge #" + cur.id + " It does not follow the previous contribution");
        return false;
    }
    sr = await sameRatio$1(curve1, prev.alphaG1, cur.alphaG1, cur.key.alpha.g2_sp, cur.key.alpha.g2_spx);
    if (sr !== true) {
        if (logger) logger.error("INVALID alpha*G1. challenge #" + cur.id + " It does not follow the previous contribution");
        return false;
    }
    sr = await sameRatio$1(curve1, prev.betaG1, cur.betaG1, cur.key.beta.g2_sp, cur.key.beta.g2_spx);
    if (sr !== true) {
        if (logger) logger.error("INVALID beta*G1. challenge #" + cur.id + " It does not follow the previous contribution");
        return false;
    }
    sr = await sameRatio$1(curve1, cur.key.beta.g1_s, cur.key.beta.g1_sx, prev.betaG2, cur.betaG2);
    if (sr !== true) {
...
async function verify(tauFilename, logger) {
    ...
    const { curve: curve1, power, ceremonyPower } = await readPTauHeader(fd, sections);
    const contrs = await readContributions(fd, curve1, sections);
    ...
    const initialContribution = {
        ...
        betaG1: curve1.G1.g,
        betaG2: curve1.G2.g,
        nextChallenge: calculateFirstChallengeHash(curve1, ceremonyPower, logger),
    ...
    const res = await verifyContribution(curve1, curContr, prevContr, logger);
    if (!res) return false;
    ...
    sr = await sameRatio$1(curve1, rTau1.R1, rTau1.R2, curve1.G2.g, curContr.tauG2);
    if (sr !== true) {
        if (logger) logger.error("tauG1 section. Powers do not match");
        return false;
    }
    if (!curve1.G1.eq(curve1.G1.g, rTau1.singularPoints[0])) {
        if (logger) logger.error("First element of tau*G1 section must be the generator");
        return false;
    }
    if (!curve1.G1.eq(curContr.tauG1, rTau1.singularPoints[1])) {
    ...
    sr = await sameRatio$1(curve1, curve1.G1.g, curContr.tauG1, rTau2.R1, rTau2.R2);
    if (sr !== true) {
        if (logger) logger.error("tauG2 section. Powers do not match");
        return false;
    }
    if (!curve1.G2.eq(curve1.G2.g, rTau2.singularPoints[0])) {
        if (logger) logger.error("First element of tau*G2 section must be the generator");
        return false;
    }
    if (!curve1.G2.eq(curContr.tauG2, rTau2.singularPoints[1])) {
    ...
    sr = await sameRatio$1(curve1, rAlphaTauG1.R1, rAlphaTauG1.R2, curve1.G2.g, curContr.tauG2);
    if (sr !== true) {
        if (logger) logger.error("alphaTauG1 section. Powers do not match");
        return false;
    }
    if (!curve1.G1.eq(curContr.alphaG1, rAlphaTauG1.singularPoints[0])) {
    ...
    sr = await sameRatio$1(curve1, rBetaTauG1.R1, rBetaTauG1.R2, curve1.G2.g, curContr.tauG2);
    if (sr !== true) {
        if (logger) logger.error("betaTauG1 section. Powers do not match");
        return false;
    }
    if (!curve1.G1.eq(curContr.betaG1, rBetaTauG1.singularPoints[0])) {
        if (logger) logger.error("First element of beta*tau*G1 section (beta*G1) does not match the one in the contribution section");
        return false;
    }
    //Verify Beta G2
    const betaG2 = await processSectionBetaG2(logger);
    if (!curve1.G2.eq(curContr.betaG2, betaG2)) {
    ...
    function printContribution(curContr, prevContr) {
        ...
        const buffV = new Uint8Array(curve1.G1.F.n8 * 2 * 6 + curve1.G2.F.n8 * 2 * 3);
        toPtauPubKeyRpr(buffV, 0, curve1, curContr.key, false);
        const responseHasher = blake2bWasm.exports(64);
    ...
    async function verifyLagrangeEvaluations(gName, tauSection, lagrangeSection, sectionName, logger) {
        ...
        async function verifyPower(p) {
            ...
            const n8r = curve1.Fr.n8;
            const nPoints = 2 ** p;
            let buff_r = new Uint32Array(nPoints);
            let buffG;
            let rng = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ChaCha"](seed);
            ...
            buffG = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](nPoints * sG);
            if (p == power + 1) {
                await fd.readToBuffer(buffG, 0, (nPoints - 1) * sG);
                buffG.set(curve1.G1.zeroAffine, (nPoints - 1) * sG);
            } else {
                await fd.readToBuffer(buffG, 0, nPoints * sG);
            }
            await endReadSection(fd, true);
            const resTau = await G.multiExpAffine(buffG, buff_r, logger, sectionName + "_" + p);
            buff_r = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](nPoints * n8r);
            rng = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["ChaCha"](seed);
            ...
            buff_r = await curve1.Fr.batchToMontgomery(buff_r);
            if (logger) logger.debug(`fft ${p}...`);
            buff_r = await curve1.Fr.fft(buff_r);
            if (logger) logger.debug(`batchFromMontgomery ${p}...`);
            buff_r = await curve1.Fr.batchFromMontgomery(buff_r);
...
*/ async function applyKeyToSection(fdOld, sections, fdNew, idSection, curve1, groupName, first, inc, sectionName, logger) {
    const MAX_CHUNK_SIZE = 1 << 16;
    const G = curve1[groupName];
    const sG = G.F.n8 * 2;
    ...
    for(let i = 0; i < nPoints; i += MAX_CHUNK_SIZE){
        ...
        t = curve1.Fr.mul(t, curve1.Fr.exp(inc, n));
    }
    await endWriteSection(fdNew);
    await endReadSection(fdOld);
}
async function applyKeyToChallengeSection(fdOld, fdNew, responseHasher, curve1, groupName, nPoints, first, inc, formatOut, sectionName, logger) {
    const G = curve1[groupName];
    const sG = G.F.n8 * 2;
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ async function challengeContribute(curve1, challengeFilename, responseFileName, entropy, logger) {
    await blake2bWasm.exports.ready();
    const fdFrom = await readExisting(challengeFilename);
    const sG1 = curve1.F1.n64 * 8 * 2;
    const sG2 = curve1.F2.n64 * 8 * 2;
    ...
    if (logger) {
        [
            ...
            logger.debug(k + ".g1_s: " + curve1.G1.toString(key[k].g1_s, 16));
            logger.debug(k + ".g1_sx: " + curve1.G1.toString(key[k].g1_sx, 16));
            logger.debug(k + ".g2_sp: " + curve1.G2.toString(key[k].g2_sp, 16));
            logger.debug(k + ".g2_spx: " + curve1.G2.toString(key[k].g2_spx, 16));
            logger.debug("");
        });
    }
    const responseHasher = blake2bWasm.exports(64);
    ...
    await applyKeyToChallengeSection(fdFrom, fdTo, responseHasher, curve1, "G1", 2 ** power * 2 - 1, curve1.Fr.one, key.tau.prvKey, "COMPRESSED", "tauG1", logger);
    await applyKeyToChallengeSection(fdFrom, fdTo, responseHasher, curve1, "G2", 2 ** power, curve1.Fr.one, key.tau.prvKey, "COMPRESSED", "tauG2", logger);
    await applyKeyToChallengeSection(fdFrom, fdTo, responseHasher, curve1, "G1", 2 ** power, key.alpha.prvKey, key.tau.prvKey, "COMPRESSED", "alphaTauG1", logger);
    await applyKeyToChallengeSection(fdFrom, fdTo, responseHasher, curve1, "G1", 2 ** power, key.beta.prvKey, key.tau.prvKey, "COMPRESSED", "betaTauG1", logger);
    await applyKeyToChallengeSection(fdFrom, fdTo, responseHasher, curve1, "G2", 1, key.beta.prvKey, key.tau.prvKey, "COMPRESSED", "betaTauG2", logger);
    // Write and hash key
    const buffKey = new Uint8Array(curve1.F1.n8 * 2 * 6 + curve1.F2.n8 * 2 * 3);
    toPtauPubKeyRpr(buffKey, 0, curve1, key, false);
...
*/ async function beacon$1(oldPtauFilename, newPTauFilename, name, beaconHashStr, numIterationsExp, logger) {
    ...
    await blake2bWasm.exports.ready();
    const { fd: fdOld, sections } = await readBinFile(oldPtauFilename, "ptau", 1);
    const { curve: curve1, power, ceremonyPower } = await readPTauHeader(fdOld, sections);
    if (power != ceremonyPower) {
    ...
    curContribution.key = await keyFromBeacon(curve1, lastChallengeHash, beaconHash, numIterationsExp);
    ...
    const startSections = [];
    let firstPoints;
    firstPoints = await processSection(2, "G1", 2 ** power * 2 - 1, curve1.Fr.e(1), curContribution.key.tau.prvKey, "tauG1", logger);
    curContribution.tauG1 = firstPoints[1];
    firstPoints = await processSection(3, "G2", 2 ** power, curve1.Fr.e(1), curContribution.key.tau.prvKey, "tauG2", logger);
    ...
    const buffKey = new Uint8Array(curve1.F1.n8 * 2 * 6 + curve1.F2.n8 * 2 * 3);
    toPtauPubKeyRpr(buffKey, 0, curve1, curContribution.key, false);
...
*/ async function contribute(oldPtauFilename, newPTauFilename, name, entropy, logger) {
    ...
    curContribution.key = createPTauKey(curve1, lastChallengeHash, rng);
    const responseHasher = new blake2bWasm.exports(64);
    responseHasher.update(lastChallengeHash);
    const fdNew = await createBinFile(newPTauFilename, "ptau", 1, 7);
    await writePTauHeader(fdNew, curve1, power);
    const startSections = [];
    let firstPoints;
    firstPoints = await processSection(2, "G1", 2 ** power * 2 - 1, curve1.Fr.e(1), curContribution.key.tau.prvKey, "tauG1");
    curContribution.tauG1 = firstPoints[1];
    firstPoints = await processSection(3, "G2", 2 ** power, curve1.Fr.e(1), curContribution.key.tau.prvKey, "tauG2");
    curContribution.tauG2 = firstPoints[1];
...
*/ async function preparePhase2(oldPtauFilename, newPTauFilename, logger) {
    ...
    async function processSection(oldSectionId, newSectionId, Gstr, sectionName) {
        ...
        async function processSectionPower(p) {
            ...
            const sGin = G.F.n8 * 2;
            let buff;
            buff = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](nPoints * sGin);
            await startReadUniqueSection(fdOld, sections, oldSectionId);
            if (oldSectionId == 2 && p == power + 1) {
                await fdOld.readToBuffer(buff, 0, (nPoints - 1) * sGin);
                buff.set(curve1.G1.zeroAffine, (nPoints - 1) * sGin);
            } else {
...
*/ async function truncate(ptauFilename, template, logger) {
    ...
    const { curve: curve1, power, ceremonyPower } = await readPTauHeader(fdOld, sections);
    const sG1 = curve1.G1.F.n8 * 2;
    const sG2 = curve1.G2.F.n8 * 2;
    ...
    async function generateTruncate(p) {
        ...
        await writePTauHeader(fdNew, curve1, p, ceremonyPower);
...
*/ async function exportJson(pTauFilename, verbose) {
    ...
    const { curve: curve1, power } = await readPTauHeader(fd, sections);
    const pTau = {};
    pTau.q = curve1.q;
    pTau.power = power;
    pTau.contributions = await readContributions(fd, curve1, sections);
    ...
    await fd.close();
    return stringifyBigIntsWithField(curve1.Fr, pTau);
    "TURBOPACK unreachable";
    async function exportSection(sectionId, groupName, nPoints, sectionName) {
        const G = curve1[groupName];
        const sG = G.F.n8 * 2;
...
async function readR1csHeader(fd, sections, singleThread) {
    ...
    if (options.F) {
        ...
        try {
            res.curve = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["getCurveFromR"])(res.prime, options.singleThread);
            res.F = res.curve.Fr;
        } catch (err) {
            res.F = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["F1Field"](res.prime);
        }
    }
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const bls12381r = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e("73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001", 16);
const bn128r = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e("21888242871839275222246405745257275088548364400416034343698204186575808495617");
async function r1csInfo(r1csName, logger) {
    const cir = await readR1cs(r1csName);
    if (__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(cir.prime, bn128r)) {
        if (logger) logger.info("Curve: bn-128");
    } else if (__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(cir.prime, bls12381r)) {
        if (logger) logger.info("Curve: bls12-381");
    } else {
        if (logger) logger.info(`Unknown Curve. Prime: ${__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].toString(cir.prime)}`);
    }
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$7 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
*/ async function wtnsCheck(r1csFilename, wtnsFilename, logger) {
    ...
    const { fd: fdWtns, sections: wtnsSections } = await readBinFile(wtnsFilename, "wtns", 2);
    const wtnsHeader = await readHeader(fdWtns, wtnsSections);
    if (!__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(r1cs.prime, wtnsHeader.q)) {
        throw new Error("Curve of the witness does not match the curve of the proving key");
    }
    const buffWitness = await readSection(fdWtns, wtnsSections, 2);
    await fdWtns.close();
    const curve1 = await getCurveFromR(r1cs.prime);
    const Fr = curve1.Fr;
    const sFr = Fr.n8;
...
*/ async function newZKey(r1csName, ptauName, zkeyName, logger) {
    ...
    const { curve: curve1, power } = await readPTauHeader(fdPTau, sectionsPTau);
    ...
    const primeR = curve1.r;
    const n8r = (Math.floor((__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].bitLength(primeR) - 1) / 64) + 1) * 8;
    const Rr = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].mod(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].shl(1, n8r * 8), primeR);
    const R2r = curve1.Fr.e(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].mod(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].mul(Rr, Rr), primeR));
    ...
    bAlpha1 = await curve1.G1.batchLEMtoU(bAlpha1);
    csHasher.update(bAlpha1);
    let bBeta1;
    bBeta1 = await fdPTau.read(sG1, sectionsPTau[5][0].p);
    await fdZKey.write(bBeta1);
    bBeta1 = await curve1.G1.batchLEMtoU(bBeta1);
    csHasher.update(bBeta1);
    let bBeta2;
    bBeta2 = await fdPTau.read(sG2, sectionsPTau[6][0].p);
    await fdZKey.write(bBeta2);
    bBeta2 = await curve1.G2.batchLEMtoU(bBeta2);
    csHasher.update(bBeta2);
    const bg1 = new Uint8Array(sG1);
    curve1.G1.toRprLEM(bg1, 0, curve1.G1.g);
    const bg2 = new Uint8Array(sG2);
    curve1.G2.toRprLEM(bg2, 0, curve1.G2.g);
    const bg1U = new Uint8Array(sG1);
    curve1.G1.toRprUncompressed(bg1U, 0, curve1.G1.g);
    const bg2U = new Uint8Array(sG2);
    curve1.G2.toRprUncompressed(bg2U, 0, curve1.G2.g);
    ...
    async function writeHs() {
        ...
        const buffOut = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](domainSize * sG1);
        if (cirPower < curve1.Fr.s) {
    ...
    async function processConstraints() {
        const buffCoeff = new Uint8Array(12 + curve1.Fr.n8);
        const buffCoeffV = new DataView(buffCoeff.buffer);
        const bOne = new Uint8Array(curve1.Fr.n8);
        curve1.Fr.toRprLE(bOne, 0, curve1.Fr.e(1));
        let r1csPos = 0;
        ...
        const buffSection = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](coefs.length * (12 + curve1.Fr.n8) + 4);
        const buff4 = new Uint8Array(4);
        const buff4V = new DataView(buff4.buffer);
        ...
        function writeCoef(c) {
            ...
            let n;
            if (c[3] >= 0) {
                n = curve1.Fr.fromRprLE(sR1cs.slice(c[3], c[3] + curve1.Fr.n8), 0);
            } else {
                n = curve1.Fr.fromRprLE(bOne, 0);
            }
            const nR2 = curve1.Fr.mul(n, R2r);
            curve1.Fr.toRprLE(buffCoeff, 12, nR2);
    ...
    async function composeAndWritePoints(idSection, groupName, arr, sectionName) {
        ...
        while(i < arr.length){
            let t = 0;
            while(i < arr.length && t < curve1.tm.concurrency){
    ...
    async function composeAndWritePointsThread(groupName, arr, logger, sectionName) {
        ...
        let bBases, bScalars;
        if (acc > 2 << 14) {
            bBases = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](acc * sGin);
            bScalars = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](acc * curve1.Fr.n8);
        } else {
            bBases = new Uint8Array(acc * sGin);
            bScalars = new Uint8Array(acc * curve1.Fr.n8);
        }
        let pB = 0;
        let pS = 0;
        const sBuffs = [
        ...
        for(let i = 0; i < arr.length; i++){
            ...
            for(let j = 0; j < arr[i].length; j++){
                ...
                if (arr[i][j][2] >= 0) {
                    bScalars.set(sR1cs.slice(arr[i][j][2], arr[i][j][2] + curve1.Fr.n8), offset * curve1.Fr.n8);
                } else {
                    bScalars.set(bOne, offset * curve1.Fr.n8);
                }
                offset++;
            }
        }
        if (arr.length > 1) {
            ...
            const res = await curve1.tm.queueAction(task);
            return res;
        } else {
    ...
    async function hashHPointsChunk(offset, nPoints) {
        ...
        const concurrency = curve1.tm.concurrency;
...
async function phase2exportMPCParams(zkeyName, mpcparamsName, logger) {
    ...
    const sG1 = curve1.G1.F.n8 * 2;
    const sG2 = curve1.G2.F.n8 * 2;
    const mpcParams = await readMPCParams(fdZKey, curve1, sectionsZKey);
    ...
    buffBasesIC = await curve1.G1.batchLEMtoU(buffBasesIC);
    ...
    let buffBasesH_Tau;
    buffBasesH_Tau = await curve1.G1.fft(buffBasesH_Lodd, "affine", "jacobian", logger);
    buffBasesH_Tau = await curve1.G1.batchApplyKey(buffBasesH_Tau, curve1.Fr.neg(curve1.Fr.e(2)), curve1.Fr.w[zkey.power + 1], "jacobian", "affine", logger);
    // Remove last element.  (The degree of H will be always m-2)
    buffBasesH_Tau = buffBasesH_Tau.slice(0, buffBasesH_Tau.byteLength - sG1);
    buffBasesH_Tau = await curve1.G1.batchLEMtoU(buffBasesH_Tau);
    ...
    buffBasesC = await curve1.G1.batchLEMtoU(buffBasesC);
    ...
    buffBasesA = await curve1.G1.batchLEMtoU(buffBasesA);
    ...
    buffBasesB1 = await curve1.G1.batchLEMtoU(buffBasesB1);
    ...
    buffBasesB2 = await curve1.G2.batchLEMtoU(buffBasesB2);
...
*/ async function phase2importMPCParams(zkeyNameOld, mpcparamsName, zkeyNameNew, name, logger) {
    ...
    const curve1 = await getCurveFromQ(zkeyHeader.q);
    const sG1 = curve1.G1.F.n8 * 2;
    const sG2 = curve1.G2.F.n8 * 2;
    const oldMPCParams = await readMPCParams(fdZKeyOld, curve1, sectionsZKeyOld);
    const newMPCParams = {};
    ...
    const buffTauLEM = await curve1.G1.batchUtoLEM(buffTauU);
    buffH = new Uint8Array(zkeyHeader.domainSize * sG1);
    buffH.set(buffTauLEM); // Let the last one to zero.
    curve1.G1.toRprLEM(buffH, sG1 * (zkeyHeader.domainSize - 1), curve1.G1.zeroAffine);
    const n2Inv = curve1.Fr.neg(curve1.Fr.inv(curve1.Fr.e(2)));
    const wInv = curve1.Fr.inv(curve1.Fr.w[zkeyHeader.power + 1]);
    buffH = await curve1.G1.batchApplyKey(buffH, n2Inv, wInv, "affine", "jacobian", logger);
    buffH = await curve1.G1.ifft(buffH, "jacobian", "affine", logger);
    ...
    buffL = await curve1.G1.batchUtoLEM(buffL);
    ...
    await writeMPCParams(fdZKeyNew, curve1, newMPCParams);
    await fdMPCParams.close();
    ...
    async function readG1(fd) {
        const buff = await fd.read(curve1.G1.F.n8 * 2);
        return curve1.G1.fromRprUncompressed(buff, 0);
    }
    async function readG2(fd) {
        const buff = await fd.read(curve1.G2.F.n8 * 2);
        return curve1.G2.fromRprUncompressed(buff, 0);
    }
    function contributionIsEqual(c1, c2) {
        if (!curve1.G1.eq(c1.deltaAfter, c2.deltaAfter)) return false;
        if (!curve1.G1.eq(c1.delta.g1_s, c2.delta.g1_s)) return false;
        if (!curve1.G1.eq(c1.delta.g1_sx, c2.delta.g1_sx)) return false;
        if (!curve1.G2.eq(c1.delta.g2_spx, c2.delta.g2_spx)) return false;
...
async function phase2verifyFromInit(initFileName, pTauFileName, zkeyFileName, logger) {
    ...
    const curve1 = await getCurveFromQ(zkey.q);
    const sG1 = curve1.G1.F.n8 * 2;
    const mpcParams = await readMPCParams(fd, curve1, sections);
    const accumulatedHasher = blake2bWasm.exports(64);
    accumulatedHasher.update(mpcParams.csHash);
    let curDelta = curve1.G1.g;
    for(let i = 0; i < mpcParams.contributions.length; i++){
        const c = mpcParams.contributions[i];
        const ourHasher = cloneHasher(accumulatedHasher);
        hashG1(ourHasher, curve1, c.delta.g1_s);
        hashG1(ourHasher, curve1, c.delta.g1_sx);
        if (!hashIsEqual(ourHasher.digest(), c.transcript)) {
            console.log(`INVALID(${i}): Inconsistent transcript `);
            return false;
        }
        const delta_g2_sp = hashToG2(curve1, c.transcript);
        sr = await sameRatio(curve1, c.delta.g1_s, c.delta.g1_sx, delta_g2_sp, c.delta.g2_spx);
        if (sr !== true) {
            console.log(`INVALID(${i}): public key G1 and G2 do not have the same ration `);
            return false;
        }
        sr = await sameRatio(curve1, curDelta, c.deltaAfter, delta_g2_sp, c.delta.g2_spx);
        if (sr !== true) {
        ...
        if (c.type == 1) {
            ...
            const expected_prvKey = curve1.Fr.fromRng(rng);
            const expected_g1_s = curve1.G1.toAffine(curve1.G1.fromRng(rng));
            const expected_g1_sx = curve1.G1.toAffine(curve1.G1.timesFr(expected_g1_s, expected_prvKey));
            if (curve1.G1.eq(expected_g1_s, c.delta.g1_s) !== true) {
                console.log(`INVALID(${i}): Key of the beacon does not match. g1_s `);
                return false;
            }
            if (curve1.G1.eq(expected_g1_sx, c.delta.g1_sx) !== true) {
                console.log(`INVALID(${i}): Key of the beacon does not match. g1_sx `);
                return false;
            }
        }
        hashPubKey(accumulatedHasher, curve1, c);
        const contributionHasher = blake2bWasm.exports(64);
        hashPubKey(contributionHasher, curve1, c);
    ...
    if (!__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(zkeyInit.q, zkey.q) || !__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].eq(zkeyInit.r, zkey.r) || zkeyInit.n8q != zkey.n8q || zkeyInit.n8r != zkey.n8r) {
    ...
    if (!curve1.G1.eq(zkey.vk_alpha_1, zkeyInit.vk_alpha_1)) {
        if (logger) logger.error("INVALID:  Invalid alpha1");
        return false;
    }
    if (!curve1.G1.eq(zkey.vk_beta_1, zkeyInit.vk_beta_1)) {
        if (logger) logger.error("INVALID:  Invalid beta1");
        return false;
    }
    if (!curve1.G2.eq(zkey.vk_beta_2, zkeyInit.vk_beta_2)) {
        if (logger) logger.error("INVALID:  Invalid beta2");
        return false;
    }
    if (!curve1.G2.eq(zkey.vk_gamma_2, zkeyInit.vk_gamma_2)) {
        if (logger) logger.error("INVALID:  Invalid gamma2");
        return false;
    }
    if (!curve1.G1.eq(zkey.vk_delta_1, curDelta)) {
        if (logger) logger.error("INVALID:  Invalid delta1");
        return false;
    }
    sr = await sameRatio(curve1, curve1.G1.g, curDelta, curve1.G2.g, zkey.vk_delta_2);
    if (sr !== true) {
        if (logger) logger.error("INVALID:  Invalid delta2");
        return false;
    }
    const mpcParamsInit = await readMPCParams(fdInit, curve1, sectionsInit);
    ...
    async function sectionHasSameRatio(groupName, fd1, sections1, fd2, sections2, idSection, g2sp, g2spx, sectionName) {
        ...
        sr = await sameRatio(curve1, R1, R2, g2sp, g2spx);
    ...
    async function sameRatioH() {
        ...
        let buff_r = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](zkey.domainSize * zkey.n8r);
        const seed = new Array(8);
        for(let i = 0; i < 8; i++){
            seed[i] = readUInt32BE(getRandomBytes(4), 0);
        }
        ...
        await endReadSection(fd);
        sr = await sameRatio(curve1, R1, R2, zkey.vk_delta_2, zkeyInit.vk_delta_2);
        if (sr !== true) return false;
        return true;
    }
    async function batchSubtract(buff1, buff2) {
        const sG = curve1.G1.F.n8 * 2;
...
*/ async function phase2contribute(zkeyNameOld, zkeyNameNew, name, entropy, logger) {
    ...
    const curve1 = await getCurveFromQ(zkey.q);
    const mpcParams = await readMPCParams(fdOld, curve1, sections);
    const fdNew = await createBinFile(zkeyNameNew, "zkey", 1, 10);
    const rng = await getRandomRng(entropy);
    const transcriptHasher = blake2bWasm.exports(64);
    transcriptHasher.update(mpcParams.csHash);
    for(let i = 0; i < mpcParams.contributions.length; i++){
        hashPubKey(transcriptHasher, curve1, mpcParams.contributions[i]);
    }
    const curContribution = {};
    curContribution.delta = {};
    curContribution.delta.prvKey = curve1.Fr.fromRng(rng);
    curContribution.delta.g1_s = curve1.G1.toAffine(curve1.G1.fromRng(rng));
    curContribution.delta.g1_sx = curve1.G1.toAffine(curve1.G1.timesFr(curContribution.delta.g1_s, curContribution.delta.prvKey));
    hashG1(transcriptHasher, curve1, curContribution.delta.g1_s);
    hashG1(transcriptHasher, curve1, curContribution.delta.g1_sx);
    curContribution.transcript = transcriptHasher.digest();
    curContribution.delta.g2_sp = hashToG2(curve1, curContribution.transcript);
    curContribution.delta.g2_spx = curve1.G2.toAffine(curve1.G2.timesFr(curContribution.delta.g2_sp, curContribution.delta.prvKey));
    zkey.vk_delta_1 = curve1.G1.timesFr(zkey.vk_delta_1, curContribution.delta.prvKey);
    zkey.vk_delta_2 = curve1.G2.timesFr(zkey.vk_delta_2, curContribution.delta.prvKey);
    ...
    // B2 Section
    await copySection(fdOld, sections, fdNew, 7);
    const invDelta = curve1.Fr.inv(curContribution.delta.prvKey);
    await applyKeyToSection(fdOld, sections, fdNew, 8, curve1, "G1", invDelta, curve1.Fr.e(1), "L Section", logger);
    await applyKeyToSection(fdOld, sections, fdNew, 9, curve1, "G1", invDelta, curve1.Fr.e(1), "H Section", logger);
    await writeMPCParams(fdNew, curve1, mpcParams);
    await fdOld.close();
    await fdNew.close();
    const contributionHasher = blake2bWasm.exports(64);
    hashPubKey(contributionHasher, curve1, curContribution);
...
async function zkeyExportJson(zkeyFileName) {
    ...
    delete zKey.curve;
    delete zKey.F;
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"].stringifyBigInts(zKey);
}
/*
    Copyright 2018 0KIMS association.
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ async function bellmanContribute(curve1, challengeFilename, responseFileName, entropy, logger) {
    await blake2bWasm.exports.ready();
    const rng = await getRandomRng(entropy);
    const delta = curve1.Fr.fromRng(rng);
    const invDelta = curve1.Fr.inv(delta);
    const sG1 = curve1.G1.F.n8 * 2;
    const sG2 = curve1.G2.F.n8 * 2;
    const fdFrom = await readExisting(challengeFilename);
    const fdTo = await createOverride(responseFileName);
    ...
    const delta1 = curve1.G1.timesFr(oldDelta1, delta);
    await writeG1(delta1);
    const oldDelta2 = await readG2();
    const delta2 = curve1.G2.timesFr(oldDelta2, delta);
    ...
    await applyKeyToChallengeSection(fdFrom, fdTo, null, curve1, "G1", nH, invDelta, curve1.Fr.e(1), "UNCOMPRESSED", "H", logger);
    // L
    const nL = await fdFrom.readUBE32();
    await fdTo.writeUBE32(nL);
    await applyKeyToChallengeSection(fdFrom, fdTo, null, curve1, "G1", nL, invDelta, curve1.Fr.e(1), "UNCOMPRESSED", "L", logger);
    // A
    ...
    for(let i = 0; i < nContributions; i++){
        ...
        hashPubKey(transcriptHasher, curve1, c);
    }
    const curContribution = {};
    curContribution.delta = {};
    curContribution.delta.prvKey = delta;
    curContribution.delta.g1_s = curve1.G1.toAffine(curve1.G1.fromRng(rng));
    curContribution.delta.g1_sx = curve1.G1.toAffine(curve1.G1.timesFr(curContribution.delta.g1_s, delta));
    hashG1(transcriptHasher, curve1, curContribution.delta.g1_s);
    hashG1(transcriptHasher, curve1, curContribution.delta.g1_sx);
    curContribution.transcript = transcriptHasher.digest();
    curContribution.delta.g2_sp = hashToG2(curve1, curContribution.transcript);
    curContribution.delta.g2_spx = curve1.G2.toAffine(curve1.G2.timesFr(curContribution.delta.g2_sp, delta));
    ...
    async function readG1() {
        const buff = await fdFrom.read(curve1.G1.F.n8 * 2);
        return curve1.G1.fromRprUncompressed(buff, 0);
    }
    async function readG2() {
        const buff = await fdFrom.read(curve1.G2.F.n8 * 2);
        return curve1.G2.fromRprUncompressed(buff, 0);
    }
    async function writeG1(P) {
        const buff = new Uint8Array(sG1);
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { stringifyBigInts: stringifyBigInts$3 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
async function groth16Vk(zkey, fd, sections) {
    const curve1 = await getCurveFromQ(zkey.q);
    const sG1 = curve1.G1.F.n8 * 2;
    const alphaBeta = await curve1.pairing(zkey.vk_alpha_1, zkey.vk_beta_2);
    let vKey = {
        protocol: zkey.protocol,
        curve: curve1.name,
        nPublic: zkey.nPublic,
        vk_alpha_1: curve1.G1.toObject(zkey.vk_alpha_1),
        vk_beta_2: curve1.G2.toObject(zkey.vk_beta_2),
        vk_gamma_2: curve1.G2.toObject(zkey.vk_gamma_2),
        vk_delta_2: curve1.G2.toObject(zkey.vk_delta_2),
        vk_alphabeta_12: curve1.Gt.toObject(alphaBeta)
    };
    // Read IC Section
    ///////////
    await startReadUniqueSection(fd, sections, 3);
    ...
    for(let i = 0; i <= zkey.nPublic; i++){
        ...
        const P = curve1.G1.toObject(buff);
        vKey.IC.push(P);
    }
    await endReadSection(fd);
...
async function plonkVk(zkey) {
    ...
    let vKey = {
        ...
        nPublic: zkey.nPublic,
        power: zkey.power,
        k1: curve1.Fr.toObject(zkey.k1),
        k2: curve1.Fr.toObject(zkey.k2),
        Qm: curve1.G1.toObject(zkey.Qm),
        Ql: curve1.G1.toObject(zkey.Ql),
        Qr: curve1.G1.toObject(zkey.Qr),
        Qo: curve1.G1.toObject(zkey.Qo),
        Qc: curve1.G1.toObject(zkey.Qc),
        S1: curve1.G1.toObject(zkey.S1),
        S2: curve1.G1.toObject(zkey.S2),
        S3: curve1.G1.toObject(zkey.S3),
        X_2: curve1.G2.toObject(zkey.X_2),
        w: curve1.Fr.toObject(curve1.Fr.w[zkey.power])
    };
    vKey = stringifyBigInts$3(vKey);
    return vKey;
}
async function exportFFlonkVk(zkey, logger) {
    ...
    let vKey = {
        ...
        w3: curve1.Fr.toObject(zkey.w3),
        w4: curve1.Fr.toObject(zkey.w4),
        w8: curve1.Fr.toObject(zkey.w8),
        wr: curve1.Fr.toObject(zkey.wr),
        X_2: curve1.G2.toObject(zkey.X_2),
        C0: curve1.G1.toObject(zkey.C0)
    };
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$6, stringifyBigInts: stringifyBigInts$2 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
async function fflonkExportSolidityVerifier(vk, templates, logger) {
    ...
    const curve1 = await getCurveFromName(vk.curve);
    // Precompute w3_2, w4_2 and w4_3
    let w3 = fromVkey(vk.w3);
    vk.w3_2 = toVkey(curve1.Fr.square(w3));
    let w4 = fromVkey(vk.w4);
    vk.w4_2 = toVkey(curve1.Fr.square(w4));
    vk.w4_3 = toVkey(curve1.Fr.mul(curve1.Fr.square(w4), w4));
    let w8 = fromVkey(vk.w8);
    let acc = curve1.Fr.one;
    for(let i = 1; i < 8; i++){
        acc = curve1.Fr.mul(acc, w8);
        vk["w8_" + i] = toVkey(acc);
    }
    ...
    function fromVkey(str) {
        ...
        return curve1.Fr.fromObject(val);
    }
    function toVkey(val) {
        const str = curve1.Fr.toObject(val);
...
*/ async function plonkSetup(r1csName, ptauName, zkeyName, logger) {
    ...
    const nPublic = r1cs.nOutputs + r1cs.nPubInputs;
    await processConstraints(curve1.Fr, r1cs, logger);
    if (globalThis.gc) {
        globalThis.gc();
    }
    ...
    const LPoints = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](domainSize * sG1);
    ...
    const buffOut = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((domainSize + 6) * sG1);
    ...
    async function writeQMap(sectionNum, posConstraint, name) {
        let Q = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](domainSize * n8r);
        for(let i = 0; i < plonkConstraints.length; i++){
        ...
        vk[name] = await curve1.G1.multiExpAffine(LPoints, Q, logger, "multiexp " + name);
    }
    async function writeP4(buff) {
        const q = await Fr.ifft(buff);
        const q4 = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](domainSize * n8r * 4);
        q4.set(q, 0);
    ...
    async function writeSigma(sectionNum, name) {
        const sigma = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](n8r * domainSize * 3);
        ...
        vk.S1 = await curve1.G1.multiExpAffine(LPoints, S1, logger, "multiexp S1");
        if (globalThis.gc) {
            globalThis.gc();
        }
        vk.S2 = await curve1.G1.multiExpAffine(LPoints, S2, logger, "multiexp S2");
        if (globalThis.gc) {
            globalThis.gc();
        }
        vk.S3 = await curve1.G1.multiExpAffine(LPoints, S3, logger, "multiexp S3");
        if (globalThis.gc) {
            globalThis.gc();
        }
    ...
    async function writeLs(sectionNum, name) {
        ...
        for(let i = 0; i < l; i++){
            let buff = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](domainSize * n8r);
...
class Keccak256Transcript {
    ...
    getChallenge() {
        ...
        const value = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromRprBE(new Uint8Array(keccak256.arrayBuffer(buffer)));
...
*/ class Polynomial {
    ...
    static fromCoefficientsArray(array, curve1, logger) {
        const Fr = curve1.Fr;
        let buff = array.length > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](array.length * Fr.n8) : new Uint8Array(array.length * Fr.n8);
    ...
    static fromPolynomial(polynomial, curve1, logger) {
        ...
        let Fr = curve1.Fr;
        let buff = length > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](length * Fr.n8) : new Uint8Array(length * Fr.n8);
    ...
    blindCoefficients(blindingFactors) {
        ...
        const blindedCoefficients = this.length() + blindingFactors.length > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((this.length() + blindingFactors.length) * this.Fr.n8) : new Uint8Array((this.length() + blindingFactors.length) * this.Fr.n8);
    ...
    static async to4T(buffer, domainSize, blindingFactors, Fr) {
        ...
        const a4 = domainSize * 4 > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](domainSize * 4 * Fr.n8) : new Uint8Array(domainSize * 4 * Fr.n8);
        a4.set(a, 0);
        ...
        const a1 = domainSize + blindingFactors.length > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((domainSize + blindingFactors.length) * Fr.n8) : new Uint8Array((domainSize + blindingFactors.length) * Fr.n8);
        a1.set(a, 0);
        for(let i = 0; i < blindingFactors.length; i++){
    ...
    byXSubValue(value) {
        ...
        const buff = length > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](length * Fr.n8) : new Uint8Array(length * Fr.n8);
    ...
    divBy(polynomial) {
        ...
        this.coef = this.length() > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](this.length() * Fr.n8) : new Uint8Array(this.length() * Fr.n8);
    ...
    divByMonic(m, beta) {
        const Fr = this.Fr;
        let d = this.degree();
        let buffer = this.length() > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](this.length() * Fr.n8) : new Uint8Array(this.length() * Fr.n8);
    ...
    fastDivByVanishing(data) {
        ...
        for(let i = 0; i < data.length; i++){
            ...
            let polTmp = new Polynomial(this.length() > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](this.length() * Fr.n8) : new Uint8Array(this.length() * Fr.n8), this.curve, this.logger);
            let ptr = this.coef;
    ...
    divByXSubValue(value) {
        const coefs = this.length() > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](this.length() * this.Fr.n8) : new Uint8Array(this.length() * this.Fr.n8);
    ...
    // }
    byX() {
        const coefs = this.length() + 1 > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](this.coef.byteLength + this.Fr.n8) : new Uint8Array(this.coef.byteLength + this.Fr.n8);
    ...
    static async expX(polynomial, n, truncate = false) {
        ...
        const bufferDst = length * n + 1 > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((length * n + 1) * Fr.n8) : new Uint8Array((length * n + 1) * Fr.n8);
    ...
    split(numPols, degPols, blindingFactors) {
        ...
        for(let i = 0; i < numPols; i++){
            ...
            let buff = byteLength / this.Fr.n8 > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](byteLength) : new Uint8Array(byteLength);
    ...
    truncate() {
        ...
        if (deg + 1 < this.coef.byteLength / this.Fr.n8) {
            const newCoefs = deg + 1 > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((deg + 1) * this.Fr.n8) : new Uint8Array((deg + 1) * this.Fr.n8);
        ...
        function computeLagrangePolynomial(i) {
            ...
            for(let j = 0; j < xArr.length; j++){
                ...
                if (polynomial === undefined) {
                    let buff = xArr.length > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](xArr.length * Fr.n8) : new Uint8Array(xArr.length * Fr.n8);
    ...
    static zerofierPolynomial(xArr, curve1) {
        const Fr = curve1.Fr;
        let buff = xArr.length + 1 > 2 << 14 ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((xArr.length + 1) * Fr.n8) : new Uint8Array((xArr.length + 1) * Fr.n8);
...
/*
    ...
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ class Evaluations {
    constructor(evaluations, curve1, logger){
        this.eval = evaluations;
        this.curve = curve1;
        this.Fr = curve1.Fr;
        this.logger = logger;
    }
    static async fromPolynomial(polynomial, extension, curve1, logger) {
        const coefficientsN = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](polynomial.length() * extension * curve1.Fr.n8);
        coefficientsN.set(polynomial.coef, 0);
        const evaluations = await curve1.Fr.fft(coefficientsN);
        return new Evaluations(evaluations, curve1, logger);
    }
    getEvaluation(index) {
        const i_n8 = index * this.Fr.n8;
...
/*
    ...
    You should have received a copy of the GNU General Public License along with
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ const { stringifyBigInts: stringifyBigInts$1 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
async function plonk16Prove(zkeyFileName, witnessFileName, logger, options) {
    ...
    if (logger) {
        ...
        logger.debug(`  Curve:         ${curve1.name}`);
    ...
    const buffInternalWitness = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](n8r * zkey.nAdditions);
    let buffers = {};
    let polynomials = {};
    let evaluations = {};
    let challenges = {};
    let proof = new Proof(curve1, logger);
    const transcript = new Keccak256Transcript(curve1);
    ...
    polynomials.Sigma1 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
    polynomials.Sigma2 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
    polynomials.Sigma3 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
    ...
    evaluations.Sigma1 = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
    evaluations.Sigma2 = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
    evaluations.Sigma3 = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
    ...
    for(let i = 1; i <= zkey.nPublic; i++){
        ...
        publicSignals.push(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromRprLE(pub));
    }
    ...
    async function round1() {
        ...
        challenges.b = [];
        for(let i = 1; i <= 11; i++){
            challenges.b[i] = curve1.Fr.random();
        }
    ...
    async function computeWirePolynomials() {
        ...
        buffers.A = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain);
        buffers.B = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain);
        buffers.C = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain);
        ...
        polynomials.A = await Polynomial.fromEvaluations(buffers.A, curve1, logger);
        if (logger) logger.debug("··· Computing B ifft");
        polynomials.B = await Polynomial.fromEvaluations(buffers.B, curve1, logger);
        if (logger) logger.debug("··· Computing C ifft");
        polynomials.C = await Polynomial.fromEvaluations(buffers.C, curve1, logger);
        // Compute extended evaluations of a(X), b(X) and c(X) polynomials
        if (logger) logger.debug("··· Computing A fft");
        evaluations.A = await Evaluations.fromPolynomial(polynomials.A, 4, curve1, logger);
        if (logger) logger.debug("··· Computing B fft");
        evaluations.B = await Evaluations.fromPolynomial(polynomials.B, 4, curve1, logger);
        if (logger) logger.debug("··· Computing C fft");
        evaluations.C = await Evaluations.fromPolynomial(polynomials.C, 4, curve1, logger);
    ...
    async function computeZ() {
        ...
        let numArr = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain);
        let denArr = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain);
        ...
        polynomials.Z = await Polynomial.fromEvaluations(buffers.Z, curve1, logger);
        // Compute extended evaluations of z(X) polynomial
        if (logger) logger.debug("··· Computing Z fft");
        evaluations.Z = await Evaluations.fromPolynomial(polynomials.Z, 4, curve1, logger);
    ...
    async function computeT() {
        if (logger) logger.debug(`··· Reading sections ${ZKEY_PL_QL_SECTION}, ${ZKEY_PL_QR_SECTION}` + `, ${ZKEY_PL_QM_SECTION}, ${ZKEY_PL_QO_SECTION}, ${ZKEY_PL_QC_SECTION}. Q selectors`);
        // Reserve memory for Q's evaluations
        evaluations.QL = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
        evaluations.QR = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
        evaluations.QM = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
        evaluations.QO = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
        evaluations.QC = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4), curve1, logger);
        ...
        evaluations.Lagrange = new Evaluations(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4 * zkey.nPublic), curve1, logger);
        for(let i = 0; i < zkey.nPublic; i++){
        ...
        buffers.T = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4);
        buffers.Tz = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4);
        ...
        polynomials.T = await Polynomial.fromEvaluations(buffers.T, curve1, logger);
        ...
        polynomials.Tz = await Polynomial.fromEvaluations(buffers.Tz, curve1, logger);
        ...
        polynomials.T1 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((zkey.domainSize + 1) * n8r), curve1, logger);
        polynomials.T2 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((zkey.domainSize + 1) * n8r), curve1, logger);
        polynomials.T3 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((zkey.domainSize + 6) * n8r), curve1, logger);
    ...
    async function computeR() {
        ...
        polynomials.QL = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
        polynomials.QR = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
        polynomials.QM = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
        polynomials.QO = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
        polynomials.QC = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain), curve1, logger);
        // Read Q's evaluations from zkey file
        ...
        polynomials.R = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((zkey.domainSize + 6) * n8r), curve1, logger);
        ...
        let tmp = Polynomial.fromPolynomial(polynomials.T3, curve1, logger);
    ...
    function computeWxi() {
        polynomials.Wxi = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain + 6 * n8r), curve1, logger);
    ...
    async function computeWxiw() {
        polynomials.Wxiw = Polynomial.fromPolynomial(polynomials.Z, curve1, logger);
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$5 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
/*
    ...
    You should have received a copy of the GNU General Public License along with
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$4 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
async function plonkVerify(_vk_verifier, _publicSignals, _proof, logger) {
    ...
    let proof = fromObjectProof(curve1, _proof);
    vk_verifier = fromObjectVk$1(curve1, vk_verifier);
    if (!isWellConstructed(curve1, proof)) {
    ...
    const challenges = calculatechallenges(curve1, proof, publicSignals, vk_verifier);
    if (logger) {
    ...
    const L = calculateLagrangeEvaluations(curve1, challenges, vk_verifier);
    if (logger) {
        for(let i = 1; i < L.length; i++){
            logger.debug(`L${i}(xi)=` + Fr.toString(L[i], 16));
        }
    ...
    const pi = calculatePI$1(curve1, publicSignals, L);
    if (logger) {
        logger.debug("PI(xi): " + Fr.toString(pi, 16));
    }
    const r0 = calculateR0(curve1, proof, challenges, pi, L[1]);
    if (logger) {
        logger.debug("r0: " + Fr.toString(r0, 16));
    }
    const D = calculateD(curve1, proof, challenges, vk_verifier, L[1]);
    if (logger) {
        logger.debug("D: " + G1.toString(G1.toAffine(D), 16));
    }
    const F = calculateF(curve1, proof, challenges, vk_verifier, D);
    if (logger) {
        logger.debug("F: " + G1.toString(G1.toAffine(F), 16));
    }
    const E = calculateE(curve1, proof, challenges, r0);
    if (logger) {
        logger.debug("E: " + G1.toString(G1.toAffine(E), 16));
    }
    const res = await isValidPairing$1(curve1, proof, challenges, vk_verifier, E, F);
    if (logger) {
        if (res) {
            logger.info("OK!");
        } else {
...
function fromObjectProof(curve1, proof) {
    const G1 = curve1.G1;
    const Fr = curve1.Fr;
    const res = {};
...
function fromObjectVk$1(curve1, vk) {
    const G1 = curve1.G1;
    const G2 = curve1.G2;
    const Fr = curve1.Fr;
    const res = vk;
...
function isWellConstructed(curve1, proof) {
    const G1 = curve1.G1;
...
function calculatechallenges(curve1, proof, publicSignals, vk) {
    const Fr = curve1.Fr;
    const res = {};
    const transcript = new Keccak256Transcript(curve1);
...
function calculateLagrangeEvaluations(curve1, challenges, vk) {
    const Fr = curve1.Fr;
...
function calculatePI$1(curve1, publicSignals, L) {
    const Fr = curve1.Fr;
    let pi = Fr.zero;
    for(let i = 0; i < publicSignals.length; i++){
        const w = Fr.e(publicSignals[i]);
...
function calculateR0(curve1, proof, challenges, pi, l1) {
    const Fr = curve1.Fr;
    const e1 = pi;
...
function calculateD(curve1, proof, challenges, vk, l1) {
    const G1 = curve1.G1;
    const Fr = curve1.Fr;
...
function calculateF(curve1, proof, challenges, vk, D) {
    const G1 = curve1.G1;
...
function calculateE(curve1, proof, challenges, r0) {
    const G1 = curve1.G1;
    const Fr = curve1.Fr;
...
async function isValidPairing$1(curve1, proof, challenges, vk, E, F) {
    const G1 = curve1.G1;
    const Fr = curve1.Fr;
    let A1 = proof.Wxi;
    ...
    B1 = G1.add(B1, F);
    B1 = G1.sub(B1, E);
    const res = await curve1.pairingEq(G1.neg(A1), vk.X_2, B1, curve1.G2.one);
    return res;
}
/*
    Copyright 2021 0KIMS association.
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$3 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
function p256$1(n) {
    let nstr = n.toString(16);
    while(nstr.length < 64)nstr = "0" + nstr;
    nstr = `"0x${nstr}"`;
...
*/ class CPolynomial {
    ...
    getPolynomial() {
        ...
        let polynomial = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](lengthBuffer * sFr), this.curve, this.logger);
...
*/ async function fflonkSetup(r1csFilename, ptauFilename, zkeyFilename, logger) {
    ...
    const { curve: curve1 } = await readPTauHeader(fdPTau, pTauSections);
    // Read r1cs file
    if (logger) logger.info("> Reading r1cs file");
    const { fd: fdR1cs, sections: sectionsR1cs } = await readBinFile(r1csFilename, "r1cs", 1);
    const r1cs = await readR1csFd(fdR1cs, sectionsR1cs, {
    ...
    await computeFFConstraints(curve1.Fr, r1cs, logger);
    if (globalThis.gc) globalThis.gc();
    ...
    const wr = getOmegaCubicRoot(settings.cirPower, curve1.Fr);
    ...
    async function writeQMap(fdZKey, sectionNum, posConstraint, name) {
        ...
        let Q = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](settings.domainSize * sFr);
        for(let i = 0; i < plonkConstraints.length; i++){
        ...
        polynomials[name] = await Polynomial.fromEvaluations(Q, curve1, logger);
        evaluations[name] = await Evaluations.fromPolynomial(polynomials[name], 4, curve1, logger);
    ...
    async function writeSigma(fdZKey) {
        // Compute sigma
        const sigma = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sFr * settings.domainSize * 3);
        ...
        for(let i = 0; i < 3; i++){
            ...
            let name = "S" + (i + 1);
            polynomials[name] = await Polynomial.fromEvaluations(sigma.slice(settings.domainSize * sFr * i, settings.domainSize * sFr * (i + 1)), curve1, logger);
            evaluations[name] = await Evaluations.fromPolynomial(polynomials[name], 4, curve1, logger);
    ...
    async function writeLagrangePolynomials(fdZKey) {
        ...
        for(let i = 0; i < l; i++){
            let buff = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](settings.domainSize * sFr);
    ...
    async function writePtau(fdZKey) {
        ...
        PTau = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"]((settings.domainSize * 9 + 18) * sG1);
    ...
    async function writeC0(fdZKey) {
        ...
        let C0 = new CPolynomial(8, curve1, logger);
    ...
    function computeW3() {
        ...
        let exponent = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].div(orderRsub1, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].e(3));
...
/*
    ...
    You should have received a copy of the GNU General Public License along with
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ const { stringifyBigInts } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
async function fflonkProve(zkeyFileName, witnessFileName, logger, options) {
    if (logger) logger.info("FFLONK PROVER STARTED");
    // Read witness file
    if (logger) logger.info("> Reading witness file");
    const { fd: fdWtns, sections: wtnsSections } = await readBinFile(witnessFileName, "wtns", 2);
    ...
    // We set it to zero to go faster in the exponentiations.
    buffWitness.set(Fr.zero, 0);
    const buffInternalWitness = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](zkey.nAdditions * sFr);
    let buffers = {};
    let polynomials = {};
    let evaluations = {};
    ...
    const PTau = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](zkey.domainSize * 16 * sG1);
    ...
    async function round1() {
        ...
        async function computeT0() {
            ...
            evaluations.lagrange1 = new Evaluations(lagrangePolynomials, curve1, logger);
            // Reserve memory for buffers T0
            buffers.T0 = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4);
            ...
            polynomials.T0 = await Polynomial.fromEvaluations(buffers.T0, curve1, logger);
        ...
        async function computeC1() {
            let C1 = new CPolynomial(4, curve1, logger);
    ...
    async function round2() {
        ...
        async function computeT1() {
            ...
            buffers.T1 = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 2);
            buffers.T1z = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 2);
            // Set initial omega
            let omega = Fr.one;
            ...
            polynomials.T1 = await Polynomial.fromEvaluations(buffers.T1, curve1, logger);
            ...
            polynomials.T1z = await Polynomial.fromEvaluations(buffers.T1z, curve1, logger);
            // Add the polynomial T1z to T1 to get the final polynomial T1
        ...
        async function computeT2() {
            ...
            buffers.T2 = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4);
            buffers.T2z = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 4);
            // Set initial omega
            let omega = Fr.one;
            for(let i = 0; i < zkey.domainSize * 4; i++){
            ...
            polynomials.T2 = await Polynomial.fromEvaluations(buffers.T2, curve1, logger);
            ...
            polynomials.T2z = await Polynomial.fromEvaluations(buffers.T2z, curve1, logger);
        ...
        async function computeC2() {
            let C2 = new CPolynomial(3, curve1, logger);
    ...
    async function round4() {
        ...
        polynomials.C0 = new Polynomial(new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](sDomain * 8), curve1, logger);
        ...
        async function computeF() {
            if (logger) logger.info("··· Computing F polynomial");
            // COMPUTE F(X)
            polynomials.F = Polynomial.fromPolynomial(polynomials.C0, curve1, logger);
            polynomials.F.sub(polynomials.R0);
            polynomials.F.divByZerofier(8, challenges.xi);
            let f2 = Polynomial.fromPolynomial(polynomials.C1, curve1, logger);
            f2.sub(polynomials.R1);
            f2.mulScalar(challenges.alpha);
            f2.divByZerofier(4, challenges.xi);
            let f3 = Polynomial.fromPolynomial(polynomials.C2, curve1, logger);
    ...
    async function round5() {
        ...
        async function computeL() {
            ...
            // COMPUTE L(X)
            polynomials.L = Polynomial.fromPolynomial(polynomials.C0, curve1, logger);
            polynomials.L.subScalar(evalR0Y);
            polynomials.L.mulScalar(preL0);
            let l2 = Polynomial.fromPolynomial(polynomials.C1, curve1, logger);
            l2.subScalar(evalR1Y);
            l2.mulScalar(preL1);
            let l3 = Polynomial.fromPolynomial(polynomials.C2, curve1, logger);
    ...
    function getMontgomeryBatchedInverse() {
        ...
        computeLiS0(toInverse, roots.S0.h0w8, challenges.y, curve1);
        computeLiS1(toInverse, roots.S1.h1w4, challenges.y, curve1);
        computeLiS2(toInverse, roots.S2.h2w3, roots.S2.h3w3, challenges.y, challenges.xi, challenges.xiw, curve1);
        //   · L_i i=1 to num public inputs, needed in step 6 and 7 of the verifier to compute L_1(xi) and PI(xi)
        ...
        "TURBOPACK unreachable";
        function computeLiS0(toInverse, roots, x, curve1) {
            const Fr = curve1.Fr;
        ...
        function computeLiS1(toInverse, roots, x, curve1) {
            const Fr = curve1.Fr;
            const len = roots.length;
            const den1 = Fr.mul(Fr.e(len), Fr.exp(roots[0], len - 2));
            const Li = [];
        ...
        function computeLiS2(toInverse, S2, S2p, value, xi, xiw, curve1) {
            const Fr = curve1.Fr;
            const Li = [];
...
/*
    ...
    You should have received a copy of the GNU General Public License along with
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$2 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
...
/*
    ...
    You should have received a copy of the GNU General Public License along with
    snarkjs. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts: unstringifyBigInts$1 } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
async function fflonkVerify(_vk_verifier, _publicSignals, _proof, logger) {
    ...
    const curve1 = await getCurveFromName(_vk_verifier.curve);
    const vk = fromObjectVk(curve1, _vk_verifier);
    // TODO ??? Compute wr^3 and check if it matches with w
    const proof = new Proof(curve1, logger);
    ...
    if (!publicInputsAreValid(curve1, publicSignals)) {
        if (logger) logger.error("Public inputs are not valid.");
        return false;
    }
    ...
    const { challenges, roots } = computeChallenges(curve1, proof, vk, publicSignals, logger);
    ...
    const lagrangeEvals = await computeLagrangeEvaluations(curve1, challenges, vk);
    // STEP 7 - Compute public input evaluation PI(xi)
    if (logger) logger.info("> Computing polynomial identities PI(X)");
    const pi = calculatePI(curve1, publicSignals, lagrangeEvals);
    // STEP 8 - Compute polynomial r0 ∈ F_{<4}[X]
    if (logger) logger.info("> Computing r0(y)");
    const r0 = computeR0(proof, challenges, roots, curve1, logger);
    // STEP 9 - Compute polynomial r1 ∈ F_{<4}[X]
    if (logger) logger.info("> Computing r1(y)");
    const r1 = computeR1(proof, challenges, roots, pi, curve1, logger);
    // STEP 9 - Compute polynomial r2 ∈ F_{<6}[X]
    if (logger) logger.info("> Computing r2(y)");
    const r2 = computeR2(proof, challenges, roots, lagrangeEvals[1], vk, curve1, logger);
    if (logger) logger.info("> Computing F");
    const F = computeF(curve1, proof, vk, challenges, roots);
    if (logger) logger.info("> Computing E");
    const E = computeE(curve1, proof, challenges, vk, r0, r1, r2);
    if (logger) logger.info("> Computing J");
    const J = computeJ(curve1, proof, challenges);
    if (logger) logger.info("> Validate all evaluations with a pairing");
    const res = await isValidPairing(curve1, proof, challenges, vk, F, E, J);
    if (logger) {
        if (res) {
...
function fromObjectVk(curve1, vk) {
    const res = vk;
    res.k1 = curve1.Fr.fromObject(vk.k1);
    res.k2 = curve1.Fr.fromObject(vk.k2);
    res.w = curve1.Fr.fromObject(vk.w);
    // res.wW = curve.Fr.fromObject(vk.wW);
    res.w3 = curve1.Fr.fromObject(vk.w3);
    res.w4 = curve1.Fr.fromObject(vk.w4);
    res.w8 = curve1.Fr.fromObject(vk.w8);
    res.wr = curve1.Fr.fromObject(vk.wr);
    res.X_2 = curve1.G2.fromObject(vk.X_2);
    res.C0 = curve1.G1.fromObject(vk.C0);
    return res;
}
function commitmentsBelongToG1(curve1, proof, vk) {
    const G1 = curve1.G1;
...
function checkValueBelongToField(curve1, value) {
    return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].lt(value, curve1.r);
}
function checkEvaluationIsValid(curve1, evaluation) {
    return checkValueBelongToField(curve1, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["Scalar"].fromRprLE(evaluation));
}
function evaluationsAreValid(curve1, proof) {
    return checkEvaluationIsValid(curve1, proof.evaluations.ql) && checkEvaluationIsValid(curve1, proof.evaluations.qr) && checkEvaluationIsValid(curve1, proof.evaluations.qm) && checkEvaluationIsValid(curve1, proof.evaluations.qo) && checkEvaluationIsValid(curve1, proof.evaluations.qc) && checkEvaluationIsValid(curve1, proof.evaluations.s1) && checkEvaluationIsValid(curve1, proof.evaluations.s2) && checkEvaluationIsValid(curve1, proof.evaluations.s3) && checkEvaluationIsValid(curve1, proof.evaluations.a) && checkEvaluationIsValid(curve1, proof.evaluations.b) && checkEvaluationIsValid(curve1, proof.evaluations.c) && checkEvaluationIsValid(curve1, proof.evaluations.z) && checkEvaluationIsValid(curve1, proof.evaluations.zw) && checkEvaluationIsValid(curve1, proof.evaluations.t1w) && checkEvaluationIsValid(curve1, proof.evaluations.t2w);
}
function publicInputsAreValid(curve1, publicInputs) {
    for(let i = 0; i < publicInputs.length; i++){
        if (!checkValueBelongToField(curve1, publicInputs[i])) {
            return false;
        }
    }
    return true;
}
function computeChallenges(curve1, proof, vk, publicSignals, logger) {
    const Fr = curve1.Fr;
    const challenges = {};
    const roots = {};
    const transcript = new Keccak256Transcript(curve1);
...
async function computeLagrangeEvaluations(curve1, challenges, vk) {
    const Fr = curve1.Fr;
    const size = Math.max(1, vk.nPublic);
    const numArr = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](size * Fr.n8);
    let denArr = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["BigBuffer"](size * Fr.n8);
    let w = Fr.one;
    for(let i = 0; i < size; i++){
        const i_sFr = i * Fr.n8;
...
function calculatePI(curve1, publicSignals, lagrangeEvals) {
    const Fr = curve1.Fr;
    let pi = Fr.zero;
    for(let i = 0; i < publicSignals.length; i++){
        const w = Fr.e(publicSignals[i]);
...
function computeR0(proof, challenges, roots, curve1, logger) {
    const Fr = curve1.Fr;
    const Li = computeLagrangeLiSi(roots.S0.h0w8, challenges.y, challenges.xi, curve1);
...
function computeR1(proof, challenges, roots, pi, curve1, logger) {
    const Fr = curve1.Fr;
    const Li = computeLagrangeLiSi(roots.S1.h1w4, challenges.y, challenges.xi, curve1);
...
function computeR2(proof, challenges, roots, lagrange1, vk, curve1, logger) {
    const Fr = curve1.Fr;
    const LiS2 = computeLagrangeLiS2([
        roots.S2.h2w3,
        roots.S2.h3w3
    ], challenges.y, challenges.xi, challenges.xiw, curve1);
...
function computeF(curve1, proof, vk, challenges, roots) {
    const G1 = curve1.G1;
    const Fr = curve1.Fr;
...
function computeE(curve1, proof, challenges, vk, r0, r1, r2) {
    const G1 = curve1.G1;
    const Fr = curve1.Fr;
...
function computeJ(curve1, proof, challenges) {
    const G1 = curve1.G1;
    return G1.timesFr(proof.polynomials.W1, challenges.temp);
}
async function isValidPairing(curve1, proof, challenges, vk, F, E, J) {
    const G1 = curve1.G1;
    let A1 = G1.timesFr(proof.polynomials.W2, challenges.y);
    A1 = G1.add(G1.sub(G1.sub(F, E), J), A1);
    const A2 = curve1.G2.one;
    const B1 = proof.polynomials.W2;
    const B2 = vk.X_2;
    return await curve1.pairingEq(G1.neg(A1), A2, B1, B2);
}
function computeLagrangeLiSi(roots, x, xi, curve1) {
    const Fr = curve1.Fr;
    const len = roots.length;
...
function computeLagrangeLiS2(roots, value, xi0, xi1, curve1) {
    const Fr = curve1.Fr;
    const Li = [];
...
/*
    ...
    You should have received a copy of the GNU General Public License
    along with snarkJS. If not, see <https://www.gnu.org/licenses/>.
*/ const { unstringifyBigInts } = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ffjavascript$2f$build$2f$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["utils"];
function p256(n) {
    let nstr = n.toString(16);
    while(nstr.length < 64)nstr = "0" + nstr;
    nstr = `0x${nstr}`;
...
//# sourceMappingURL=node_modules_snarkjs_build_browser_esm_f676f4.js.map