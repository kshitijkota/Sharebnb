  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const onChainCommitments = await commitmentStorage.getCommitments(
          userId
        );
        ...
        const commitmentExists = onChainCommitments.some(
          (commitment) =>
            commitment.toString().toLowerCase() ===
            calculatedCommitment.toLowerCase()
        );

        if (!commitmentExists) {
          return res
            .status(400)
            .json({ error: "Commitment verification failed" });
        }
